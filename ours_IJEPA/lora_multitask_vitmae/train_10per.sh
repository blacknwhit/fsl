...
# Project root that contains the `ours_IJEPA/lora_multitask_vitmae/` package.
PROJECT_ROOT_DEFAULT="${REPO_ROOT}"
PROJECT_ROOT="${PROJECT_ROOT:-${PROJECT_ROOT_DEFAULT}}"

if [[ -d "${PROJECT_ROOT}" && -f "${PROJECT_ROOT}/ours_IJEPA/lora_multitask_vitmae/train.py" ]]; then
  cd "${PROJECT_ROOT}"
else
...
SAVE_DIR="${SAVE_DIR:-runs/lora_multitask_vitmae}"
...
TRAIN_MODULE="${TRAIN_MODULE:-ours_IJEPA.lora_multitask_vitmae.train}"
...
if [[ ! -f "${BACKBONE_CKPT}" ]]; then
  ...
  echo "  wget -c -O /data/xiangyuyue/ULLM-zf/fsl-20260209/pretrained/ijepa/IN1K-vit.h.16-448px-300e.pth.tar https://dl.fbaipublicfiles.com/ijepa/IN1K-vit.h.16-448px-300e.pth.tar" >&2
  exit 1
fi
