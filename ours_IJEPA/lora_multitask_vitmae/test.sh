...
CKPT_DEFAULT="${CKPT_DEFAULT:-${REPO_ROOT}/runs/lora_multitask_vitmae/best_combo.pt}"
CKPT="${1:-$CKPT_DEFAULT}"

PROJECT_ROOT_DEFAULT="${REPO_ROOT}"
PROJECT_ROOT="${PROJECT_ROOT:-${PROJECT_ROOT_DEFAULT}}"

if [[ -d "${PROJECT_ROOT}" && -f "${PROJECT_ROOT}/ours_IJEPA/lora_multitask_vitmae/eval.py" ]]; then
  cd "${PROJECT_ROOT}"
else
...
cmd=(
  "$PYTHON_BIN" -m "ours_IJEPA.lora_multitask_vitmae.eval"
  --checkpoint "$CKPT"
  --tasks "$TEST_TASKS"
  --stats-dir "$STATS_DIR"
  --device "$DEVICE"
