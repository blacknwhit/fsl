...
# Ensure workspace root is on sys.path so imports like
# `object_detection.*`, `segmentation.*`, `counting.*` work
...
def _import_multitask_models():
    try:
        from .models import MultiTaskModel, SharedViTMAEBackbone
    except Exception:
        ...
        from models import MultiTaskModel, SharedViTMAEBackbone
    return MultiTaskModel, SharedViTMAEBackbone
...
def parse_args():
    ...
    p.add_argument(
        ...
        type=int,
        default=64,
        help="dummy image size used only during shared IJEPA backbone init to infer embed_dim (faster; does not affect eval)",
    )
    p.add_argument("--model-name", type=str, default="ijepa_vit_huge_patch16")
