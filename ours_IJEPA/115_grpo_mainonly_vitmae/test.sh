...
CKPT_DEFAULT="${CKPT_DEFAULT:-${REPO_ROOT}/runs/115_grpo_mainonly_vitmae/best_combo.pt}"
CKPT="${1:-$CKPT_DEFAULT}"

# Project root that contains the current `ours_IJEPA/115_grpo_mainonly_vitmae/` package.
PROJECT_ROOT_DEFAULT="${REPO_ROOT}"
PROJECT_ROOT="${PROJECT_ROOT:-${PROJECT_ROOT_DEFAULT}}"

if [[ -d "${PROJECT_ROOT}" && -f "${PROJECT_ROOT}/ours_IJEPA/115_grpo_mainonly_vitmae/eval.py" ]]; then
  cd "${PROJECT_ROOT}"
else
...
EVAL_SCRIPT_DEFAULT="${PROJECT_ROOT}/ours_IJEPA/115_grpo_mainonly_vitmae/eval.py"
...
MODEL_NAME="${MODEL_NAME:-}"   # e.g. "ijepa_vit_huge_patch16"
BACKBONE_CKPT_DEFAULT="/data/xiangyuyue/ULLM-zf/fsl-20260209/pretrained/ijepa/IN1K-vit.h.16-448px-300e.pth.tar"
BACKBONE_CKPT="${BACKBONE_CKPT:-${BACKBONE_CKPT_DEFAULT}}"
...
if [[ ! -f "$EVAL_SCRIPT" ]]; then
  ...
  echo "Set EVAL_SCRIPT=/path/to/ours_IJEPA/115_grpo_mainonly_vitmae/eval.py" >&2
  exit 2
fi
