...
if [[ -d "${PROJECT_ROOT}" && -f "${PROJECT_ROOT}/ours_IJEPA/115_grpo_mainonly_vitmae/train.py" ]]; then
  cd "${PROJECT_ROOT}"
else
...
# Official IJEPA ViT-H/16 (448px, 300 epochs, IN1K) checkpoint.
OFFICIAL_BACKBONE_CKPT_DEFAULT="/data/xiangyuyue/ULLM-zf/fsl-20260209/pretrained/ijepa/IN1K-vit.h.16-448px-300e.pth.tar"
BACKBONE_CKPT="${BACKBONE_CKPT:-${OFFICIAL_BACKBONE_CKPT_DEFAULT}}"
...
MODEL_NAME="${MODEL_NAME:-ijepa_vit_huge_patch16}"
...
SAVE_DIR="${SAVE_DIR:-runs/115_grpo_mainonly_vitmae}"
...
TEST_CKPTS="${TEST_CKPTS:-${SAVE_DIR}/stage1_best.pt,${SAVE_DIR}/best_combo.pt}"
...
TEST_BACKBONE_CKPT="${TEST_BACKBONE_CKPT:-}"
...
TRAIN_MODULE="${TRAIN_MODULE:-ours_IJEPA.115_grpo_mainonly_vitmae.train}"
...
if [[ ! -f "${BACKBONE_CKPT}" ]]; then
  echo "ERROR: official IJEPA ViT-H/16 checkpoint not found: ${BACKBONE_CKPT}" >&2
  echo "Please download it first:" >&2
  echo "  mkdir -p /data/xiangyuyue/ULLM-zf/fsl-20260209/pretrained/ijepa" >&2
  echo "  wget -O /data/xiangyuyue/ULLM-zf/fsl-20260209/pretrained/ijepa/IN1K-vit.h.16-448px-300e.pth.tar https://dl.fbaipublicfiles.com/ijepa/IN1K-vit.h.16-448px-300e.pth.tar" >&2
  exit 1
fi
...
run_post_train_eval() {
  ...
  local -a env_args=(
    ...
    "BACKBONE_CKPT=${test_backbone_ckpt}"
...
run_post_train_eval_list() {
  local ckpt_list_csv="$1"
  local IFS=','
  local -a ckpt_list=()
  read -r -a ckpt_list <<< "${ckpt_list_csv}"
  ...
  for ckpt_path in "${ckpt_list[@]}"; do
    ...
    run_post_train_eval "${ckpt_path}"
  done

  if [[ "${eval_count}" -eq 0 ]]; then
    echo "[post-train] no TEST_CKPTS provided, fallback to TEST_CKPT=${TEST_CKPT}"
...
if run_training; then
  run_post_train_eval_list "${TEST_CKPTS}"
else
  train_rc=$?
