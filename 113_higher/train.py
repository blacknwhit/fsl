...
def parse_args() -> argparse.Namespace:
    ...
    p.add_argument("--state-task-hidden-dim", type=int, default=128)
...
def _denormalize_imagenet_batch(images: torch.Tensor) -> torch.Tensor:
    mean = images.new_tensor([0.485, 0.456, 0.406]).view(1, -1, 1, 1)
    std = images.new_tensor([0.229, 0.224, 0.225]).view(1, -1, 1, 1)
    return (images * std + mean).clamp(0.0, 1.0)


def _save_batch_images(images: torch.Tensor, out_dir: Path, *, prefix: str, denormalize: bool) -> None:
    out_dir.mkdir(parents=True, exist_ok=True)
    batch = images.detach().cpu()
    if batch.dim() == 3:
        batch = batch.unsqueeze(0)
    if denormalize:
        batch = _denormalize_imagenet_batch(batch)
    else:
        batch = batch.clamp(0.0, 1.0)
    for idx, img in enumerate(batch):
        save_image(img, out_dir / f"{prefix}_{idx:02d}.png")


def _save_stage1_task_batch_images(batch, out_dir: Path, *, task_name: str) -> None:
    if task_name == "det":
        images, _targets = batch
        stacked = torch.stack([img.detach().cpu() for img in images], dim=0)
        _save_batch_images(stacked, out_dir, prefix="img", denormalize=False)
        return
    if task_name == "seg":
        images, _masks = batch
        _save_batch_images(images, out_dir, prefix="img", denormalize=True)
        return
    if task_name == "cnt":
        images, _dens = batch
        _save_batch_images(images, out_dir, prefix="img", denormalize=True)
        return
    raise ValueError(f"Unknown task name for batch image save: {task_name}")


def _is_obviously_unstable_weight_vector(weights: Sequence[float]) -> bool:
    ...
    if total <= 0:
        return True
    max_ratio = max(vals) / total
    min_ratio = min(vals) / total
    return max_ratio >= 0.95 or min_ratio <= 1e-4


def _dump_stage1_unstable_batches(
    *,
    save_dir: Path,
    epoch: int,
    step: int,
    support_batches: Dict[str, object],
    meta_batches: Dict[str, object],
    weights: Sequence[float],
) -> Path:
    root = save_dir / "stage1_unstable_batches" / f"epoch_{int(epoch):03d}_step_{int(step):04d}"
    for split_name, batch_map in (("support", support_batches), ("meta", meta_batches)):
        for task_name, batch in batch_map.items():
            _save_stage1_task_batch_images(batch, root / split_name / task_name, task_name=task_name)
    info = root / "info.txt"
    info.write_text(
        "\n".join(
            [
                f"epoch={int(epoch)}",
                f"step={int(step)}",
                "weights=" + ",".join(f"{float(x):.6f}" for x in weights),
            ]
        )
        + "\n",
        encoding="utf-8",
...
def _allreduce_module_grads(module: torch.nn.Module, world_size: int) -> None:
...
def _collect_expert_param_ids(model: MultiTaskModel) -> set[int]:
    expert_param_ids: set[int] = set()
    if not bool(getattr(model.shared, "use_lora_moe", False)):
        return expert_param_ids
    for lora_moe in model.shared.lora_moes:
        expert_param_ids.update(
            {
                id(lora_moe.lora_A_private),
                id(lora_moe.lora_B_private),
                id(lora_moe.lora_A_shared),
                id(lora_moe.lora_B_shared),
            }
        )
    return expert_param_ids


def _build_generator_state_from_task_grads(
    ...
    *,
    ...
    state_a_idx: int | None,
    state_b_idx: int | None,
    device: torch.device,
    use_ddp: bool,
    world_size: int,
) -> torch.Tensor:
    task_states = []
    for task_grads in (det_task_grads, seg_task_grads, cnt_task_grads):
        task_states.append(
            state_tracker.encode_shared_grad_state(
                task_grads[state_a_idx] if state_a_idx is not None else None,
                task_grads[state_b_idx] if state_b_idx is not None else None,
                device=device,
                use_ddp=use_ddp,
...
def _weighted_task_update(
    *,
    g_det: torch.Tensor | None,
    g_seg: torch.Tensor | None,
    g_cnt: torch.Tensor | None,
    use_expert_weights: bool,
    expert_weights: torch.Tensor,
    base_weights: Sequence[float],
) -> torch.Tensor | None:
    coeffs = expert_weights if use_expert_weights else base_weights
    upd = None
    grads = (g_det, g_seg, g_cnt)
    for idx, grad in enumerate(grads):
        if grad is None:
            continue
        coeff = coeffs[idx] if use_expert_weights else float(coeffs[idx])
        term = coeff * grad
        upd = term if upd is None else upd + term
    return upd


def _assign_weighted_task_grads(
    theta_params: Sequence[torch.nn.Parameter],
    theta_is_expert: Sequence[bool],
    *,
    det_task_grads: Sequence[torch.Tensor | None],
    seg_task_grads: Sequence[torch.Tensor | None],
    cnt_task_grads: Sequence[torch.Tensor | None],
    ...
    if len(theta_params) != len(theta_is_expert):
        raise ValueError(f"theta/expert mask mismatch: {len(theta_params)} vs {len(theta_is_expert)}")
    for idx, param in enumerate(theta_params):
        upd = _weighted_task_update(
            g_det=det_task_grads[idx],
            g_seg=seg_task_grads[idx],
            g_cnt=cnt_task_grads[idx],
            use_expert_weights=bool(theta_is_expert[idx]),
            expert_weights=expert_weights,
            base_weights=base_weights,
        )
        param.grad = None if upd is None else upd.detach().clone()


def _allreduce_parameter_grads(params: Sequence[torch.nn.Parameter], world_size: int) -> None:
    if not dist.is_initialized():
        return
    for param in params:
        if param.grad is None:
...
def _weighted_task_scalar(
    det_value: torch.Tensor,
    seg_value: torch.Tensor,
    cnt_value: torch.Tensor,
    weights: Sequence[float] | torch.Tensor,
) -> torch.Tensor:
    if torch.is_tensor(weights):
        coeffs = weights
    else:
        coeffs = det_value.new_tensor([float(weights[0]), float(weights[1]), float(weights[2])])
    return coeffs[0] * det_value + coeffs[1] * seg_value + coeffs[2] * cnt_value
    ...
    *,
    ...
    for full_idx, name in enumerate(full_param_names):
        ...
        upd = _weighted_task_update(
            ...
            use_expert_weights=bool(theta_is_expert[theta_idx]),
            expert_weights=expert_weights,
            base_weights=base_weights,
        )
        if upd is None:
...
def _compute_losses(
    ...
    *,
    ...
    seg_loss = _seg_cross_entropy(seg_logits, seg_masks)
...
def _seg_cross_entropy(seg_logits: torch.Tensor, seg_masks: torch.Tensor) -> torch.Tensor:
    if seg_logits.is_cuda:
...
def _compute_seg_loss_only(model: torch.nn.Module, seg_batch, *, device: torch.device) -> torch.Tensor:
    seg_imgs, seg_masks = _to_device_seg(seg_batch, device)
    seg_logits = model("seg", seg_imgs)
    return _seg_cross_entropy(seg_logits, seg_masks)
...
def _eval_seg_loss(model: torch.nn.Module, loader, device: torch.device, *, num_classes: int, max_steps: int) -> tuple[float, float]:
    ...
    for batch in loader:
        ...
        loss = _seg_cross_entropy(logits, masks)
        bsz = imgs.size(0)
        total += float(loss.item()) * bsz
        samples += bsz
        steps += 1
...
def main() -> None:
    ...
    if not bool(args.use_lora_moe):
        raise ValueError("113_higher generator input now matches 114_grpo_stage1 and requires --use-lora-moe.")
    base_loss_weights = parse_loss_weights(args.policy_weight_prior)
    expert_param_ids = _collect_expert_param_ids(model)

    metric_logger.update_config(
        {
            "world_size": int(world_size),
            "use_ddp": int(use_ddp),
            "det_train_size": int(len(det_train_ds)),
            "det_val_size": int(len(det_val_ds)),
            "seg_train_size": int(len(seg_train_ds)),
            "seg_val_size": int(len(seg_val_ds)),
            "cnt_train_size": int(len(cnt_train_ds)),
            "cnt_val_size": int(len(cnt_val_ds)),
            "theta_param_count": int(sum(p.numel() for p in theta_params)),
        }
    )
    ...
    prior_prob = (phi_core.base_loss_weights / phi_core.base_loss_weights.sum()).detach()

    if use_ddp:
        _broadcast_module_state(model, src=0)
        _broadcast_module_state(state_tracker, src=0)
    ...
    optimizer_phi = torch.optim.AdamW(
        list(phi_core.parameters()) + list(state_tracker.parameters()),
        lr=float(args.stage1_phi_lr),
        weight_decay=0.0,
    )
    theta_named_params = [(name, p) for name, p in model.named_parameters() if p.requires_grad]
    theta_is_expert = [id(param) in expert_param_ids for _, param in theta_named_params]
    theta_index_by_id = {id(param): idx for idx, (_, param) in enumerate(theta_named_params)}
    ...
    best_phi_state = None
    best_epoch = None
    stage1_snapshot_epoch = 90
    stage1_prev_unstable = False

    if is_main and int(args.stage1_epochs) < stage1_snapshot_epoch:
        print(
            f"[warn] stage1_epochs={int(args.stage1_epochs)} < {stage1_snapshot_epoch}; "
            f"stage1 epoch-{stage1_snapshot_epoch} checkpoint will not be saved."
        )
    ...
    for epoch in range(1, int(args.stage1_epochs) + 1):
        ...
        for step, primary_batch in enumerate(train_inner_loaders[primary_inner], start=1):
            ...
            support_det_ref = feat_det.detach()
            support_det_value = float(support_det_ref.item())
            ...
            support_seg_ref = feat_seg.detach()
            support_seg_value = float(support_seg_ref.item())
            ...
            support_cnt_ref = feat_cnt.detach()
            support_cnt_value = float(support_cnt_ref.item())
            cnt_task_grads = _extract_task_grads(feat_cnt, theta_params)
            del feat_cnt
            model.zero_grad(set_to_none=True)
            stage1_lr = float(args.stage1_inner_lr)
            meta_total_value = 0.0
            improve_det_value = 0.0
            improve_seg_value = 0.0
            improve_cnt_value = 0.0
            kl_value = 0.0
            log_weights = None
            staged_fast_names: list[str] | None = None
            staged_fast_tensors: list[torch.Tensor] | None = None

            def _build_generator_state() -> torch.Tensor:
                return _build_generator_state_from_task_grads(
                    state_tracker,
                    det_task_grads=det_task_grads,
                    seg_task_grads=seg_task_grads,
                    cnt_task_grads=cnt_task_grads,
                    state_a_idx=state_a_idx,
                    state_b_idx=state_b_idx,
                    device=device,
                    use_ddp=use_ddp,
                    world_size=world_size,
                )

            expert_weights = phi_train(_build_generator_state())
            fmodel = _build_stage1_fast_model(
                model,
                device=device,
                expert_weights=expert_weights,
                base_weights=base_loss_weights,
                theta_is_expert=theta_is_expert,
            ...
            improve_det = (support_det_ref - meta_det) / (support_det_ref.abs() + 1e-6)
            improve_det_value = float(improve_det.detach().item())
            (-improve_det).backward()
            del meta_det
            del improve_det
            del fmodel, expert_weights
            ...
            improve_seg = (support_seg_ref - meta_seg) / (support_seg_ref.abs() + 1e-6)
            improve_seg_value = float(improve_seg.detach().item())
            (-improve_seg).backward()
            del meta_seg
            del improve_seg
            del fmodel, expert_weights
            ...
            improve_cnt = (support_cnt_ref - meta_cnt) / (support_cnt_ref.abs() + 1e-6)
            improve_cnt_value = float(improve_cnt.detach().item())
            (-improve_cnt).backward()
            del meta_cnt
            del improve_cnt
            del fmodel, expert_weights

            reg_weights = phi_train(_build_generator_state())
            reg_prob = reg_weights / reg_weights.sum().clamp_min(1e-8)
            kl_to_prior = (
                reg_prob
                * (
                    reg_prob.clamp_min(1e-8).log()
                    - prior_prob.to(device=reg_prob.device, dtype=reg_prob.dtype).clamp_min(1e-8).log()
                )
            ).sum()
            kl_value = float(kl_to_prior.detach().item())
            if float(args.beta_kl) > 0:
                (float(args.beta_kl) * kl_to_prior).backward()
            stage1_objective = (
                -(support_det_ref.new_tensor(improve_det_value) + support_seg_ref.new_tensor(improve_seg_value) + support_cnt_ref.new_tensor(improve_cnt_value))
                + float(args.beta_kl) * kl_to_prior.detach()
            )
            meta_total_value = float(stage1_objective.item())
            del stage1_objective
            del reg_weights, reg_prob, kl_to_prior

            if use_ddp:
                _allreduce_module_grads(state_tracker, world_size)
            ...
            copy_weights = phi_train(_build_generator_state()).detach()
            log_weights = [float(x) for x in copy_weights.cpu().tolist()]
            current_unstable = _is_obviously_unstable_weight_vector(log_weights)
            if is_main and current_unstable and not stage1_prev_unstable:
                dumped_dir = _dump_stage1_unstable_batches(
                    save_dir=save_dir,
                    epoch=epoch,
                    step=step,
                ...
                print(
                    f"[warn][stage1] unstable weight pattern detected at epoch {epoch} step {step}; "
                    f"saved current batches to {dumped_dir}"
                )
            stage1_prev_unstable = current_unstable
            copy_fmodel = _build_stage1_fast_model(
                model,
                device=device,
                expert_weights=copy_weights,
                base_weights=base_loss_weights,
                theta_is_expert=theta_is_expert,
                theta_name_to_index=theta_name_to_index,
                full_param_names=full_param_names,
            ...
            del det_task_grads, seg_task_grads, cnt_task_grads
                ...
                metric_logger.log_metrics(
                    {
                        ...
                        "stage1/meta_loss": float(meta_total_value),
                        "stage1/support_det_loss": float(support_det_value),
                        "stage1/support_seg_loss": float(support_seg_value),
                        "stage1/support_cnt_loss": float(support_cnt_value),
                        "stage1/improve_det": float(improve_det_value),
                        "stage1/improve_seg": float(improve_seg_value),
                        "stage1/improve_cnt": float(improve_cnt_value),
                        "stage1/kl_to_prior": float(kl_value),
                        "stage1/weight_det": float(w[0]),
                        "stage1/weight_seg": float(w[1]),
                        "stage1/weight_cnt": float(w[2]),
                    }
                )
        ...
        if is_main:
            stage1_epoch_loss = loss_sum / max(steps, 1)
            print(f"[stage1] epoch {epoch}/{int(args.stage1_epochs)} | meta_loss {stage1_epoch_loss:.4f}")
            metric_logger.log_metrics(
                {
                    "stage1/epoch": int(epoch),
                    "stage1/epoch_meta_loss": float(stage1_epoch_loss),
                }
            )
            if epoch == stage1_snapshot_epoch:
                stage1_epoch90 = save_dir / f"stage1_epoch{stage1_snapshot_epoch}.pt"
                save_multitask_checkpoint(
                    str(stage1_epoch90),
                    model=model,
                    optimizer=None,
                    epoch=int(epoch),
                    best_by=f"stage1_epoch{stage1_snapshot_epoch}",
                    metrics={
                        "epoch": float(epoch),
                        "stage1_epoch_loss": float(stage1_epoch_loss),
                    },
                    loss_weights=base_loss_weights,
                    phi_state={
                        "generator": _state_dict_cpu_clone(phi_core.state_dict()),
                        "state_tracker": _state_dict_cpu_clone(state_tracker.state_dict()),
                    },
                    config={
                        "stage": f"stage1_epoch{stage1_snapshot_epoch}",
                        "use_lora_moe": bool(args.use_lora_moe),
                        "unfreeze_backbone": bool(args.unfreeze_backbone),
                    },
                )
                print(f"[ckpt] saved stage1_epoch{stage1_snapshot_epoch} -> {stage1_epoch90}")
    ...
    phi_core.eval()
    for p in state_tracker.parameters():
        p.requires_grad = False
    state_tracker.eval()
    ...
    for epoch in range(1, int(args.stage2_epochs) + 1):
        ...
        for step, primary_batch in enumerate(train_full_loaders[primary_full], start=1):
            ...
            det_task_grads = _extract_task_grads(det_loss, theta_params)
            seg_task_grads = _extract_task_grads(seg_loss, theta_params)
            cnt_task_grads = _extract_task_grads(cnt_loss, theta_params)
            with torch.no_grad():
                state = _build_generator_state_from_task_grads(
                    state_tracker,
                    det_task_grads=det_task_grads,
                    seg_task_grads=seg_task_grads,
                    cnt_task_grads=cnt_task_grads,
                ...
                weights = phi_core(state)
                _assign_weighted_task_grads(
                    theta_params,
                    theta_is_expert,
                    det_task_grads=det_task_grads,
                    seg_task_grads=seg_task_grads,
            ...
            if use_ddp:
                _allreduce_parameter_grads(theta_params, world_size)
            if float(args.grad_clip_norm) > 0:
            ...
            display_total = _weighted_task_scalar(det_loss.detach(), seg_loss.detach(), cnt_loss.detach(), weights.detach()) + _weighted_task_scalar(
                det_loss.detach(),
                seg_loss.detach(),
                cnt_loss.detach(),
                base_loss_weights,
            )
            loss_sum += float(display_total.item())
            steps += 1
            ...
            if is_main and int(args.log_interval) > 0 and step % int(args.log_interval) == 0:
                ...
                print(
                    ...
                    f"loss {display_total.item():.4f} | det {det_loss.item():.4f} seg {seg_loss.item():.4f} cnt {cnt_loss.item():.4f} | "
                ...
                metric_logger.log_metrics(
                    {
                        ...
                        "stage2/loss": float(display_total.item()),
                        "stage2/det_loss": float(det_loss.detach().item()),
                        "stage2/seg_loss": float(seg_loss.detach().item()),
                        "stage2/cnt_loss": float(cnt_loss.detach().item()),
                        "stage2/weight_det": float(w[0]),
                        "stage2/weight_seg": float(w[1]),
                        "stage2/weight_cnt": float(w[2]),
                    }
                )
        ...
        if is_main:
            stage2_epoch_loss = loss_sum / max(steps, 1)
            print(f"[stage2] epoch {epoch}/{int(args.stage2_epochs)} | train {stage2_epoch_loss:.4f}")
            metric_logger.log_metrics(
                {
                    "stage2/epoch": int(epoch),
                    "stage2/epoch_train_loss": float(stage2_epoch_loss),
                }
            )
        ...
        if not bool(args.skip_validation):
            ...
            metric_logger.log_metrics(
                {
                    "val/epoch": int(epoch),
                    "val/det_loss": float(metrics["val_det_loss"]),
                    "val/seg_loss": float(metrics["val_seg_loss"]),
                    "val/seg_miou": float(metrics["val_seg_miou"]),
                    "val/cnt_loss": float(metrics["val_cnt_loss"]),
                    "val/cnt_density_mse": float(metrics["val_cnt_density_mse"]),
                    "val/cnt_mae": float(metrics["val_cnt_mae"]),
                    "val/cnt_total_mae": float(metrics["val_cnt_total_mae"]),
                    "val/ap50": float(metrics["val_ap50"]),
                    "val/selected_metric": float(metrics["selected_metric"]),
                }
            )
