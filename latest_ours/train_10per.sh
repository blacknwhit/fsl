...
SAVE_DIR="${SAVE_DIR:-runs/415_10per}"
...
if [[ "${PLAIN_LORA}" == "1" ]]; then
  ARGS+=( --lora --lora-rank "${LORA_RANK}" --lora-alpha "${LORA_ALPHA}" --lora-dropout "${LORA_DROPOUT}" --lora-weight-decay "${LORA_WEIGHT_DECAY}" )
  if [[ -n "${LORA_LR}" ]]; then
...
if [[ "${LORA_MOE}" == "1" ]]; then
  ...
  ARGS+=( --num-experts-private "${NUM_EXPERTS_PRIVATE}" --num-experts-shared "${NUM_EXPERTS_SHARED}" --moe-k-private "${MOE_K_PRIVATE}" --moe-k-shared "${MOE_K_SHARED}" )
...
mkdir -p "${SAVE_DIR}"
cp -a "${SCRIPT_PATH}" "${SAVE_DIR}/$(basename "${SCRIPT_PATH}").bak"
...
if run_training; then
  ...
  if [[ "${TEST_FINAL_CKPT}" != "${TEST_STAGE1_CKPT}" ]]; then
