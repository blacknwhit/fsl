    ...
    if is_main_process and best_state is not None:
        ...
        save_multitask_checkpoint(
            ...
            config={
                ...
                "stage1_resume_checkpoint": str(getattr(args, "stage1_resume_checkpoint", None))
                if getattr(args, "stage1_resume_checkpoint", None)
                else None,
                "loss_weights": tuple(float(x) for x in base_loss_weights),
                "use_lora_moe": bool(args.use_lora_moe),
                "use_private_experts": bool(getattr(args, "use_private_experts", True)),
                "use_shared_experts": bool(getattr(args, "use_shared_experts", True)),
...
def run_stage2_matrix_pair(
    *,
    ...
    timing_interval = max(int(getattr(args, "debug_step_timing_interval", 1)), 1)
    global_step = 0
    raw_total_matrix_units = int(len(shared_matrix_index.matrix_units))
    total_matrix_units = max(raw_total_matrix_units, 1)
    ...
    for epoch in range(1, int(args.stage2_epochs) + 1):
        ...
        for step, primary_batch in enumerate(train_loaders[primary_task], start=1):
            ...
            should_log_step_metrics = (not args.log_interval) or (step % int(args.log_interval) == 0)
            if should_log_step_metrics:
                if raw_total_matrix_units > 0:
                    print(
                        f"[stage2] epoch {epoch}/{int(args.stage2_epochs)} step {step} | "
                        f"loss {step_loss:.4f} | "
                        f"train det {step_det_loss:.4f} seg {step_seg_loss:.4f} cnt {step_cnt_loss:.4f} | "
                        f"shared_pairs det_seg {mix_stats.pair_counts['det_seg']}/{raw_total_matrix_units} "
                        f"det_cnt {mix_stats.pair_counts['det_cnt']}/{raw_total_matrix_units} "
                        f"seg_cnt {mix_stats.pair_counts['seg_cnt']}/{raw_total_matrix_units} | "
