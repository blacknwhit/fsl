...
if [[ -d "${PROJECT_ROOT}" && -f "${PROJECT_ROOT}/latest_ours/train.py" ]]; then
  cd "${PROJECT_ROOT}"
else
...
DET_TRAIN_ANN="${DET_TRAIN_ANN:-/data/xiangyuyue/ULLM-zf/fsl-20260209/data/det/coco/annotations/instances_train.json}"
...
SEG_TRAIN_DIR="${SEG_TRAIN_DIR:-/data/xiangyuyue/ULLM-zf/fsl-20260209/data/seg/nas/liyangguang103/newdataset/CD-SematicSeg/EvLab-SS/mydataset/train_5500}"
...
CNT_TRAIN_DIR="${CNT_TRAIN_DIR:-/data/xiangyuyue/ULLM-zf/fsl-20260209/data/cnt/nas/liyangguang103/newdataset/CD-Count/DSACA/train_data_class8}"
...
STAGE1_EPOCHS="${STAGE1_EPOCHS:-${EPOCHS:-120}}"
...
if [[ -n "${STAGE1_RESUME_CHECKPOINT}" ]]; then
  STAGE1_VAL_LAST_K_EPOCHS="${STAGE1_EPOCHS}"
fi
...
USE_PRIVATE_EXPERTS="${USE_PRIVATE_EXPERTS:-1}"
USE_SHARED_EXPERTS="${USE_SHARED_EXPERTS:-1}"
...
SAVE_DIR="${SAVE_DIR:-runs/ours_lora_loramoe_100per}"
...
SWANLAB_PROJECT="${SWANLAB_PROJECT:-latest_ours}"
...
TEST_STAGE1_AFTER_TRAIN="${TEST_STAGE1_AFTER_TRAIN:-1}"
...
TEST_FINAL_CKPT="${TEST_FINAL_CKPT:-${TEST_CKPT}}"
...
TRAIN_MODULE="${TRAIN_MODULE:-latest_ours.train}"
...
if [[ -n "${STAGE1_RESUME_CHECKPOINT}" ]]; then
  ARGS+=( --stage1-resume-checkpoint "${STAGE1_RESUME_CHECKPOINT}" )
fi
...
if [[ "${LORA_MOE}" == "1" ]]; then
  ...
  if [[ "${USE_PRIVATE_EXPERTS}" == "1" ]]; then
    ARGS+=( --moe-private-experts )
  else
    ARGS+=( --no-moe-private-experts )
  fi
  if [[ "${USE_SHARED_EXPERTS}" == "1" ]]; then
    ARGS+=( --moe-shared-experts )
  else
    ARGS+=( --no-moe-shared-experts )
  fi
fi
if [[ "${LORA}" == "1" ]]; then
  ARGS+=( --lora )
...
mkdir -p "${SAVE_DIR}"
cp -a "${SCRIPT_PATH}" "${SAVE_DIR}/train_100per.sh.bak"
...
if run_training; then
  if [[ "${STAGE1_ONLY}" == "1" || "${TEST_STAGE1_AFTER_TRAIN}" == "1" ]]; then
    echo "[post-train] eval stage1 best checkpoint: ${TEST_STAGE1_CKPT}"
    run_post_train_eval "${TEST_STAGE1_CKPT}"
  fi
  if [[ "${STAGE1_ONLY}" == "1" ]]; then
    echo "[post-train] stage1-only mode; skip final/stage2 eval"
    exit 0
  fi
  if [[ "${TEST_FINAL_CKPT}" != "${TEST_STAGE1_CKPT}" || "${TEST_STAGE1_AFTER_TRAIN}" != "1" ]]; then
    echo "[post-train] eval final checkpoint: ${TEST_FINAL_CKPT}"
    run_post_train_eval "${TEST_FINAL_CKPT}"
  else
    echo "[post-train] skip duplicate final eval (same as stage1): ${TEST_FINAL_CKPT}"
  fi
else
  train_rc=$?
