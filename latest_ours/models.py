...
class SharedDinoV3Backbone(nn.Module):
    """
    Shared DINOv3 backbone with optional plain LoRA and LoRA-MoE adapters.

    `use_lora` injects LoRA into DINOv3 FFN linears.
    `use_lora_moe` adds the existing parallel LoRA-MoE residual branch.
    ...
    def __init__(
        ...
        *,
        ...
        if self.use_lora_moe and not self.use_private_experts and not self.use_shared_experts:
            raise ValueError("LoRA-MoE requires at least one expert pool: private or shared")
        ...
        if backbone_trainable is None:
            ...
            backbone_trainable = not self.use_lora_moe and not self.use_lora
        ...
        if self.use_lora_moe:
            self._setup_lora_moe(
                ...
                use_private_experts=self.use_private_experts,
    ...
    def forward_features(self, x: torch.Tensor, *, trainable_override: bool | None = None, task_id: int | None = None) -> dict:
        ...
        """
        if self.use_lora:
            # Match plain-LoRA behavior: once LoRA is injected, the shared backbone path
            # should keep gradients enabled regardless of per-task freeze overrides.
