...
# Project root that contains the current `latest_ours/` package.
PROJECT_ROOT_DEFAULT="/data/xiangyuyue/ULLM-zf/fsl-20260209"
PROJECT_ROOT="${PROJECT_ROOT:-${PROJECT_ROOT_DEFAULT}}"

if [[ -d "${PROJECT_ROOT}" && -f "${PROJECT_ROOT}/latest_ours/eval.py" ]]; then
  cd "${PROJECT_ROOT}"
else
...
EVAL_SCRIPT_DEFAULT="${PROJECT_ROOT}/latest_ours/eval.py"
...
if [[ ! -f "$EVAL_SCRIPT" ]]; then
  ...
  echo "Set EVAL_SCRIPT=/path/to/latest_ours/eval.py" >&2
  exit 2
fi
...
# 浠庢潈閲嶈矾寰勬彁鍙栧悕瀛楋�?a/b/best_total.pt -> best_total
...
# i.e. the real model produced by latest_ours/train.py (including LoRA-MoE routing and optional plain LoRA).
