...
def _infer_lora_moe_config_from_shared_state(
    ...
    # Infer task_num from task-specific router indices if needed.
...
def _check_full_multitask_load_only(checkpoint_path: str, *, device: str | None, model_name: str | None, image_size: int | None) -> None:
    ...
    use_lora_moe = bool(moe_cfg["use_lora_moe"])
    use_lora = bool(plain_lora_cfg["use_lora"])
    effective_lora_rank = int(moe_cfg["lora_rank"]) if use_lora_moe else int(plain_lora_cfg["lora_rank"])

    # Defaults consistent with train.py
    name = model_name or "dinov3_vitl16"
    ...
    shared = SharedDinoV3Backbone(
        ...
        use_private_experts=bool(moe_cfg["use_private_experts"]),
        use_shared_experts=bool(moe_cfg["use_shared_experts"]),
    ...
    print(
        ...
        f"use_lora={use_lora}, use_lora_moe={use_lora_moe}, total={total_s}, matched≈{matched_s} ({(100.0*matched_s/max(total_s,1)):.1f}%), "
    ...
    if real_missing:
        print(
            "[FullLoadCheck][Warn] detector has real missing keys (not from backbone.shared.backbone.*). "
    ...
    if use_lora:
        has_lora = _state_dict_has_lora(model.shared.state_dict())
        print(f"[FullLoadCheck] plain_lora_keys_present: {has_lora}")
    if use_lora_moe:
        ...
        print(
            "[FullLoadCheck] lora_moe_keys_present: "
            f"private={has_priv}, shared={has_shared}, "
            f"use_private_experts={bool(moe_cfg['use_private_experts'])}, "
            f"use_shared_experts={bool(moe_cfg['use_shared_experts'])}"
        )
...
def _build_full_multitask_model_from_ckpt(
    ...
    *,
    ...
    missing_d, unexpected_d = model.detector.load_state_dict(det_state, strict=False)
    # Expected: det_state does not contain backbone.shared.backbone.* (filtered at save time)
    expected_prefix = "backbone.shared.backbone."
    expected_missing = [k for k in missing_d if isinstance(k, str) and k.startswith(expected_prefix)]
    real_missing = [k for k in missing_d if not (isinstance(k, str) and k.startswith(expected_prefix))]
