...
# Optional: override CLIP backbone source or local checkpoint file.
...
SAVE_DIR="${SAVE_DIR:-runs/clip_10per_lora}"
...
if [[ "${AUTO_TEST_AFTER_TRAIN}" == "1" ]]; then
  ...
  echo "[test] ${TEST_CMD[*]}"
  TEST_BACKBONE_CKPT="${TEST_BACKBONE_CKPT:-${BACKBONE_CKPT}}"
  if [[ "${LOG_TO_FILE}" == "1" ]]; then
    BACKBONE_CKPT="${TEST_BACKBONE_CKPT}" "${TEST_CMD[@]}" 2>&1 | tee -a "${LOG_FILE}"
  else
    BACKBONE_CKPT="${TEST_BACKBONE_CKPT}" "${TEST_CMD[@]}"
  fi
fi
