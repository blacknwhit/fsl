...
def _maybe_write_compat_checkpoint(checkpoint_path: str, *, stats_dir: Path) -> str:
    ...
    # If the checkpoint stores the shared CLIP wrapper state_dict(), backbone weights are under 'backbone.'
...
def _import_multitask_models():
    try:
        ...
        from models import MultiTaskModel, SharedCLIPVisionBackbone
    return MultiTaskModel, SharedCLIPVisionBackbone


def _resolve_shared_backbone_checkpoint(args) -> str | None:
    # Full-model eval builds one shared backbone; use the first provided task-level checkpoint.
    for value in (args.det_backbone_checkpoint, args.seg_backbone_checkpoint, args.cnt_backbone_checkpoint):
        if value:
            return str(value)
    return None
...
def _check_full_multitask_load_only(
    ...
    *,
    ...
    name = model_name or "openai/clip-vit-large-patch14"
    img_size = int(image_size) if image_size is not None else 448
    dev = torch.device(device) if device else torch.device("cuda" if torch.cuda.is_available() else "cpu")

    MultiTaskModel, SharedCLIPVisionBackbone = _import_multitask_models()
...
def _eval_full_model_seg(model, *, stats_dir: Path, data_dir: str, device: torch.device, num_classes: int, image_size: int) -> dict:
    ...
    mean = tuple(OPENAI_CLIP_MEAN)
    std = tuple(OPENAI_CLIP_STD)
...
def _run_eval_full_model(args) -> int:
    ...
    shared_backbone_ckpt = _resolve_shared_backbone_checkpoint(args)
    ...
    model, meta, _shared_state, _det_state, _seg_state, _cnt_state, dev = _build_full_multitask_model_from_ckpt(
        ...
        backbone_checkpoint=shared_backbone_ckpt,
    )
