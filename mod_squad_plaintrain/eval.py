...
def _maybe_write_compat_checkpoint(checkpoint_path: str, *, stats_dir: Path) -> str:
    """
    Single-task eval scripts expect checkpoint['backbone'] to be the raw DINOv3 backbone state_dict
    ...
    # If the checkpoint stores SharedDinoV3Backbone.state_dict(), backbone weights are under 'backbone.'
...
def _infer_lora_moe_config_from_shared_state(shared_state: Dict) -> Dict[str, object]:
    ...
    has_shared_lora = any(isinstance(k, str) and k.endswith("lora_A_shared") for k in shared_state.keys())
    use_lora_moe = bool(has_wrapped or has_shared_lora)
    if not use_lora_moe:
        return {
            "use_lora_moe": False,
            "task_num": 3,
    ...
    shared_a = next(
        (
            ...
            if isinstance(k, str) and k.endswith("lora_A_shared")
        ),
        None,
    )
    if hasattr(shared_a, "shape") and len(getattr(shared_a, "shape", [])) == 3:
        num_experts_shared = int(shared_a.shape[0])
        lora_rank = int(shared_a.shape[2])

    private_a = next(
        (
            v
            for k, v in shared_state.items()
            if isinstance(k, str) and k.endswith("lora_A_private")
        ),
        None,
    )
    if hasattr(private_a, "shape") and len(getattr(private_a, "shape", [])) == 4:
        task_num = int(private_a.shape[0])
        num_experts_private = int(private_a.shape[1])
        lora_rank = int(private_a.shape[3])

    gate_shared_w = next(
        (
            v
            for k, v in shared_state.items()
            if isinstance(k, str) and k.endswith("f_gate_shared.0.weight")
        ),
        None,
    )
    if hasattr(gate_shared_w, "shape") and len(getattr(gate_shared_w, "shape", [])) == 2:
        moe_k_shared = min(moe_k_shared, int(num_experts_shared))

    gate_private_w = next(
        (
            v
            for k, v in shared_state.items()
            if isinstance(k, str) and k.endswith("f_gate_private.0.weight")
        ),
        None,
    )
    if hasattr(gate_private_w, "shape") and len(getattr(gate_private_w, "shape", [])) == 2:
        moe_k_private = min(moe_k_private, int(num_experts_private))
...
def _resolve_model_config(ckpt: Dict, shared_state: Dict) -> Dict[str, object]:
    cfg = dict(ckpt.get("model_config") or {})
    inferred = _infer_lora_moe_config_from_shared_state(shared_state)
    merged = dict(inferred)
    merged.update(cfg)
    merged.setdefault("model_name", "dinov3_vitl16")
    merged.setdefault("image_size", 448)
    merged.setdefault("use_lora_moe", inferred.get("use_lora_moe", False))
    merged.setdefault("task_num", 3)
    merged.setdefault("lora_rank", 8)
    merged.setdefault("num_experts_private", 0)
    merged.setdefault("num_experts_shared", 6)
    merged.setdefault("moe_k_private", 0)
    merged.setdefault("moe_k_shared", 2)
    merged.setdefault("shared_only", True)
    return merged
...
def _check_full_multitask_load_only(checkpoint_path: str, *, device: str | None, model_name: str | None, image_size: int | None) -> None:
    ...
    cfg = _resolve_model_config(ckpt, shared_state)
    use_lora_moe = bool(cfg["use_lora_moe"])

    # Defaults consistent with train.py
    name = model_name or str(cfg.get("model_name", "dinov3_vitl16"))
    img_size = int(image_size) if image_size is not None else int(cfg.get("image_size", 448))
    ...
    if use_lora_moe:
        has_shared = any(k.endswith("lora_A_shared") for k in model.shared.state_dict().keys())
        print(f"[FullLoadCheck] lora_moe_keys_present: shared={has_shared}")
