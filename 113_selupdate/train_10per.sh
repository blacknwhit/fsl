...
if [[ -d "${PROJECT_ROOT}" && -f "${PROJECT_ROOT}/113_selupdate/train.py" ]]; then
  cd "${PROJECT_ROOT}"
else
...
SEL_PREPOS="${SEL_PREPOS:-640}"
SEL_AFFIN_DECAY="${SEL_AFFIN_DECAY:-0.0002}"
SEL_PREFERENCE="${SEL_PREFERENCE:-0.6}"
SEL_CONVERGENCE_ITER="${SEL_CONVERGENCE_ITER:-100}"
SEL_MAX_CLUSTER_RETRIES="${SEL_MAX_CLUSTER_RETRIES:-20}"
SEL_GROUP_NORM="${SEL_GROUP_NORM:-1}"

SAVE_DIR="${SAVE_DIR:-runs/113_selupdate_10per_same}"
...
TRAIN_MODULE="${TRAIN_MODULE:-113_selupdate.train}"
...
ARGS=(
  ...
  --sel-prepos "${SEL_PREPOS}"
  --sel-affin-decay "${SEL_AFFIN_DECAY}"
  --sel-preference "${SEL_PREFERENCE}"
  --sel-convergence-iter "${SEL_CONVERGENCE_ITER}"
  --sel-max-cluster-retries "${SEL_MAX_CLUSTER_RETRIES}"
)
...
if [[ "${SEL_GROUP_NORM}" == "1" ]]; then ARGS+=( --sel-group-norm ); fi
