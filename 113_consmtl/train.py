...
if __package__:
    ...
    models_module = importlib.import_module("113_consmtl.models")
    utils_module = importlib.import_module("113_consmtl.utils")
...
_SKIP_EXTRA_GRAD_TASKS: set[str] = set()
...
def parse_args():
    parser = argparse.ArgumentParser(description="Train the 113_consmtl multitask baseline")
    ...
    parser.add_argument("--save-dir", type=str, default="runs/113_consmtl")
    ...
    parser.set_defaults(det_unfreeze_backbone=False)
    ...
    parser.set_defaults(seg_full_finetune=False)
    ...
    parser.set_defaults(cnt_full_finetune=False)

    parser.add_argument("--cons-lambda", type=float, default=1.0)
    parser.add_argument("--cons-extra-clip-factor", type=float, default=0.1)
...
def _validate_args(args) -> None:
    if args.amp:
        raise SystemExit("113_consmtl v1 does not support --amp. Leave AMP=0 in the launch script.")
    if not args.lora:
        raise SystemExit("113_consmtl requires --lora because shared trainable parameters are the LoRA weights.")
    if args.unfreeze_backbone:
        raise SystemExit("113_consmtl requires the backbone to stay frozen. Do not pass --unfreeze-backbone.")
    if args.det_unfreeze_backbone:
        raise SystemExit("113_consmtl does not support detection full-backbone finetuning. Keep detection on frozen backbone + LoRA.")
    if args.seg_full_finetune:
        raise SystemExit("113_consmtl does not support segmentation full-backbone finetuning. Keep segmentation on frozen backbone + LoRA.")
    if args.cnt_full_finetune:
        raise SystemExit("113_consmtl does not support counting full-backbone finetuning. Keep counting on frozen backbone + LoRA.")
    if not (int(args.det_batch_size) == int(args.seg_batch_size) == int(args.cnt_batch_size)):
        raise SystemExit("113_consmtl requires det/seg/cnt batch sizes to be identical.")
    if float(args.cons_extra_clip_factor) < 0:
        raise SystemExit("--cons-extra-clip-factor must be >= 0.")
...
@torch.no_grad()
def _eval_seg_loss(model, loader, device: torch.device, *, num_classes: int) -> tuple[float, float]:
...
@torch.no_grad()
def _eval_cnt_loss(model, loader, device: torch.device, *, count_loss_weight: float) -> tuple[float, float, float, float]:
    model.eval()
    total = 0.0
    total_density = 0.0
    total_count_mae = 0.0
...
def _flatten_grad_list(grad_list, grad_dims: list[int], device: torch.device) -> torch.Tensor:
    flat = torch.zeros(sum(grad_dims), device=device, dtype=torch.float32)
    offset = 0
    for grad, dim in zip(grad_list, grad_dims):
        if grad is not None:
            flat[offset : offset + dim].copy_(grad.detach().reshape(-1).float())
        offset += dim
    return flat


def _overwrite_shared_grads(parameters, grad_dims: list[int], new_grad: torch.Tensor, *, n_tasks: int) -> None:
    offset = 0
    scaled_grad = new_grad * float(n_tasks)
    for parameter, dim in zip(parameters, grad_dims):
        parameter.grad = scaled_grad[offset : offset + dim].view_as(parameter).clone()
        offset += dim


def _fixed_point_consmtl(A: np.ndarray, x0: np.ndarray, max_iters: int = 256, tol: float = 1e-8) -> np.ndarray:
    x = np.clip(np.asarray(x0, dtype=np.float64), 1e-12, None)
    for _ in range(max_iters):
        Ax = np.maximum(A.dot(x), 1e-12)
        candidate = np.clip(1.0 / np.maximum(Ax * Ax, 1e-12), 1e-12, None)
...
def _solve_consmtl(shared_grad_matrix: torch.Tensor) -> np.ndarray:
    num_tasks = int(shared_grad_matrix.shape[1])
    if num_tasks <= 0:
        raise ValueError("ConsMTL requires at least one task gradient.")
    GG = shared_grad_matrix.t().mm(shared_grad_matrix).detach().cpu().numpy().astype(np.float64)
    x0 = np.full(num_tasks, 1.0 / num_tasks, dtype=np.float64)

    if scipy_least_squares is not None:
        def residual(x):
            x = np.clip(x, 1e-12, None)
            return np.dot(GG, x) - np.power(1.0 / x, 0.5)

        result = scipy_least_squares(residual, x0, bounds=(1e-12, np.inf))
        return np.clip(result.x, 1e-12, None)

    return _fixed_point_consmtl(GG, x0)


def _flatten_non_none(grads) -> torch.Tensor | None:
    tensors = [grad.reshape(-1) for grad in grads if grad is not None]
    if not tensors:
        return None
    return torch.cat(tensors)


def _assign_combined_task_grads(
    parameters,
    grad_main,
    grad_extra,
    *,
    ...
    main_flat = _flatten_non_none(grad_main)
    if main_flat is None:
        for parameter in parameters:
            parameter.grad = None
        return

    max_extra_norm = float(clip_factor) * float(main_flat.norm().item())
    extra_flat = _flatten_non_none(grad_extra)
    scale = 0.0
    if extra_flat is not None and max_extra_norm > 0.0:
        extra_norm = float(extra_flat.norm().item())
        scale = min(1.0, max_extra_norm / max(extra_norm, 1e-12))

    for parameter, grad1, grad2 in zip(parameters, grad_main, grad_extra):
        if grad1 is None and grad2 is None:
            parameter.grad = None
            continue
        combined = None
        if grad1 is not None:
            combined = grad1.detach().clone()
        if grad2 is not None and scale > 0.0:
            scaled_extra = grad2.detach() * scale
            combined = scaled_extra.clone() if combined is None else combined + scaled_extra
        parameter.grad = combined


def _compute_extra_grads_with_fallback(
    *,
    task_name: str,
    ...
    if task_name in _SKIP_EXTRA_GRAD_TASKS:
    ...
    try:
        ...
        if "roi_align" in message and ("double backward" in message or "double backwards" in message or "not supported" in message):
            _SKIP_EXTRA_GRAD_TASKS.add(task_name)
            print(
                f"[train][warn] skipping ConsMTL extra gradient for task '{task_name}' "
                "because torchvision roi_align does not support double backward. "
                "This task will use only its main gradient."
            )
            return tuple(None for _ in params)
        raise


def _ensure_matching_representation_shapes(representations: Dict[str, torch.Tensor]) -> None:
    shapes = {task_name: tuple(representation.shape) for task_name, representation in representations.items()}
    unique_shapes = set(shapes.values())
    if len(unique_shapes) != 1:
        raise RuntimeError(
            "Official ConsMTL requires task representations to have identical shapes, "
            f"but got det={shapes['det']}, seg={shapes['seg']}, cnt={shapes['cnt']}."
        )
...
def _forward_all_tasks(
    ...
    *,
    ...
    det_loss_dict, det_rep = model.forward_det(det_images, det_targets, return_representation=True)
    det_loss = sum(det_loss_dict.values())

    seg_logits, seg_rep = model.forward_seg(seg_images, return_representation=True)
    seg_loss = F.cross_entropy(seg_logits, seg_masks)

    cnt_gt_counts = cnt_density.flatten(2).sum(dim=2)
    pred_density, pred_counts, cnt_rep = model.forward_cnt(cnt_images, return_representation=True)
    ...
    batch_sizes = {int(det_rep.shape[0]), int(seg_rep.shape[0]), int(cnt_rep.shape[0])}
    if len(batch_sizes) != 1:
        raise RuntimeError(
            "ConsMTL requires equal batch sizes across tasks per step, got "
            f"det={tuple(det_rep.shape)}, seg={tuple(seg_rep.shape)}, cnt={tuple(cnt_rep.shape)}"
        )

    losses = {"det": det_loss, "seg": seg_loss, "cnt": cnt_loss}
    representations = {"det": det_rep, "seg": seg_rep, "cnt": cnt_rep}
    cnt_stats = _count_diag_stats(pred_density, cnt_density, pred_counts, cnt_gt_counts)
    return losses, representations, cnt_stats


def _compute_consmtl_step(
    *,
    model: MultiTaskModel,
    batches: Dict[str, object],
    device: torch.device,
    cnt_count_loss_weight: float,
    cons_lambda: float,
    cons_extra_clip_factor: float,
):
    losses, representations, cnt_stats = _forward_all_tasks(
        model,
        batches,
        device,
        cnt_count_loss_weight=cnt_count_loss_weight,
    )
    _ensure_matching_representation_shapes(representations)

    shared_parameters = model.shared_parameters()
    task_specific = model.task_specific_parameters()
    grad_dims = _shared_grad_dims(shared_parameters)
    ...
    for task_name in ("det", "seg", "cnt"):
        ...
        shared_grad_columns.append(_flatten_grad_list(grads, grad_dims, device))

    shared_grad_matrix = torch.stack(shared_grad_columns, dim=1)
    weights_np = _solve_consmtl(shared_grad_matrix)
    weights_tensor = torch.from_numpy(weights_np.astype(np.float32)).to(shared_grad_matrix.device)
    combined_shared_grad = (shared_grad_matrix * weights_tensor.view(1, -1)).sum(dim=1)
    _overwrite_shared_grads(shared_parameters, grad_dims, combined_shared_grad, n_tasks=3)

    deltas = {}
    for task_name in ("det", "seg", "cnt"):
        delta = torch.autograd.grad(
            losses[task_name],
            representations[task_name],
            retain_graph=True,
            create_graph=True,
        )[0]
    ...
    combined_delta = sum(
        deltas[task_name] * weights_tensor[index]
        for index, task_name in enumerate(("det", "seg", "cnt"))
    ).detach()

    for task_name in ("det", "seg", "cnt"):
        params = task_specific[task_name]
        extra_loss = -float(cons_lambda) * torch.dot(
            deltas[task_name].reshape(-1),
            combined_delta.reshape(-1),
        )
        ...
        grad_extra = _compute_extra_grads_with_fallback(
            task_name=task_name,
            extra_loss=extra_loss,
            parameters=params,
        )
        _assign_combined_task_grads(
            params,
            grad_main,
            grad_extra,
            clip_factor=cons_extra_clip_factor,
        )

    return losses, cnt_stats, weights_np
...
def main():
    ...
    if is_main_process and tuple(float(x) for x in loss_weights) != (1.0, 1.0, 1.0):
        print(
            "[train][warn] official ConsMTL uses raw task losses. "
            "--loss-weights will be kept only for logging/checkpoint compatibility."
        )
    ...
    _broadcast_module_state(model, use_ddp=use_ddp)

    shared_lora_params = model.shared_parameters()
    if not shared_lora_params:
        raise RuntimeError("No trainable shared LoRA parameters found in model.shared.backbone")
    task_params = model.task_specific_parameters()
    det_head_params = task_params["det"]
    seg_head_params = task_params["seg"]
    cnt_head_params = task_params["cnt"]
    ...
    if is_main_process:
        ...
        head_params = [parameter for parameter in trainable_params if id(parameter) not in shared_ids]
        ...
        print(f"[train] task head params: {sum(parameter.numel() for parameter in head_params)}")
    ...
    for epoch in range(1, total_epochs + 1):
        ...
        for step, primary_batch in enumerate(train_loaders[primary_task], start=1):
            ...
            losses, cnt_stats, weights_np = _compute_consmtl_step(
                model=model,
                batches=batches,
                device=device,
                cnt_count_loss_weight=float(args.cnt_count_loss_weight),
                cons_lambda=float(args.cons_lambda),
                cons_extra_clip_factor=float(args.cons_extra_clip_factor),
            )
            ...
            reduced_losses = _reduce_loss_values(losses, use_ddp=use_ddp)
            total = sum(reduced_losses[task] for task in ("det", "seg", "cnt"))
            total_loss += total
            det_loss_sum += float(reduced_losses["det"])
            seg_loss_sum += float(reduced_losses["seg"])
            cnt_loss_sum += float(reduced_losses["cnt"])
            weight_sum += weights_np.astype(np.float64)
            steps += 1

            if args.log_interval and step % int(args.log_interval) == 0:
                avg_weights = weight_sum / max(steps, 1)
                message = (
                    ...
                    f"w det={avg_weights[0]:.4f} seg={avg_weights[1]:.4f} cnt={avg_weights[2]:.4f}"
                )
                message += f" | {_format_count_diag(cnt_stats)}"
                print(message)
        ...
        val_seg, val_seg_miou = _eval_seg_loss(model, val_loaders["seg"], device, num_classes=int(args.seg_num_classes))
    ...
    if is_main_process:
        ...
        save_multitask_checkpoint(
            ...
            train_strategy="consmtl_lora_heads",
        )
