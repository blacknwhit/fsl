...
def _infer_lora_config_from_shared_state(shared_state: Dict) -> Dict:
    config = {
    ...
    _, lora_a = _find_first_tensor(shared_state, ".lora_A")
    if hasattr(lora_a, "shape") and len(lora_a.shape) >= 1:
        config["lora_rank"] = int(lora_a.shape[0])
...
def _infer_model_config(ckpt: Dict) -> Dict:
    ...
    config = _infer_lora_config_from_shared_state(shared_state)
...
def _build_model_from_checkpoint(
    ...
    *,
    ...
    model = MultiTaskModel(
        ...
        det_train_backbone=bool(config.get("det_train_backbone", False)),
        seg_train_backbone=bool(config.get("seg_train_backbone", False)),
        cnt_train_backbone=bool(config.get("cnt_train_backbone", False)),
    ).to(dev)
    model.eval()
    ...
    meta = {
        ...
        "cons_config": dict(ckpt.get("cons_config") or {}),
...
def parse_args():
    parser = argparse.ArgumentParser(description="Evaluate the 113_consmtl multitask checkpoint")
