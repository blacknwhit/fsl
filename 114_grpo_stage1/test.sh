...
CKPT_DEFAULT="${CKPT_DEFAULT:-${REPO_ROOT}/runs/grpo_10per/best_combo.pt}"
CKPT="${1:-$CKPT_DEFAULT}"

# Project root that contains the current `114_grpo_stage1/` package.
PROJECT_ROOT_DEFAULT="/data/xiangyuyue/ULLM-zf/fsl-20260209"
PROJECT_ROOT="${PROJECT_ROOT:-${PROJECT_ROOT_DEFAULT}}"

if [[ -d "${PROJECT_ROOT}" && -f "${PROJECT_ROOT}/114_grpo_stage1/eval.py" ]]; then
  cd "${PROJECT_ROOT}"
else
...
EVAL_SCRIPT_DEFAULT="${PROJECT_ROOT}/114_grpo_stage1/eval.py"
...
if [[ ! -f "$EVAL_SCRIPT" ]]; then
  ...
  echo "Set EVAL_SCRIPT=/path/to/114_grpo_stage1/eval.py" >&2
  exit 2
fi
...
# i.e. the real model produced by 114_grpo_stage1/train.py (including LoRA-MoE routing).
