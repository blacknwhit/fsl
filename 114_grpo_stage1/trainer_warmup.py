    ...
    *,
    ...
    for epoch in range(1, int(args.warmup_epochs) + 1):
        ...
        for step, primary_batch in enumerate(train_loaders[primary_task], start=1):
            ...
            total.backward()
            if manual_theta_grad_sync and not is_ddp_wrapped(model):
            ...
            if args.log_interval and step % int(args.log_interval) == 0:
                ...
                if metric_logger is not None:
                    metric_logger.log_metrics(
                        {
                            "warmup/epoch": int(epoch),
                            "warmup/step_in_epoch": int(step),
                            "warmup/step_loss": float(step_loss),
                            "warmup/step_train_det_loss": float(step_det_loss),
                            "warmup/step_train_seg_loss": float(step_seg_loss),
                            "warmup/step_train_cnt_loss": float(step_cnt_loss),
                            "warmup/step_weight_det": float(warmup_weights[0]),
                            "warmup/step_weight_seg": float(warmup_weights[1]),
                            "warmup/step_weight_cnt": float(warmup_weights[2]),
                        },
                        step=global_step,
                    )
            if debug_timing and step % timing_interval == 0:
        ...
        if metric_logger is not None:
            metric_logger.log_metrics(
                {
                    "warmup/epoch": int(epoch),
                    "warmup/epoch_loss": float(total_loss / max(steps, 1)),
                    "warmup/epoch_train_det_loss": float(det_loss_sum / max(steps, 1)),
                    "warmup/epoch_train_seg_loss": float(seg_loss_sum / max(steps, 1)),
                    "warmup/epoch_train_cnt_loss": float(cnt_loss_sum / max(steps, 1)),
                    "warmup/epoch_weight_det": float(warmup_weights[0]),
                    "warmup/epoch_weight_seg": float(warmup_weights[1]),
                    "warmup/epoch_weight_cnt": float(warmup_weights[2]),
                },
                step=int(epoch),
            )
