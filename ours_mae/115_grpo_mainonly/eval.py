...
try:
    from .vit_mae_utils import DEFAULT_MAE_MODEL_NAME, get_vit_image_stats
except ImportError:
    from vit_mae_utils import DEFAULT_MAE_MODEL_NAME, get_vit_image_stats
...
def _maybe_write_compat_checkpoint(checkpoint_path: str, *, stats_dir: Path) -> str:
    """
    Single-task eval scripts expect checkpoint['backbone'] to be the raw vision backbone state_dict
    ...
    # If the checkpoint stores the shared vision wrapper state_dict(), backbone weights are under 'backbone.'
...
def _import_multitask_models():
    try:
        from models import MultiTaskModel, SharedMAEVisionBackbone
    except Exception:
        repo_root = Path(__file__).resolve().parent
        if str(repo_root) not in sys.path:
            sys.path.insert(0, str(repo_root))
        from models import MultiTaskModel, SharedMAEVisionBackbone
    return MultiTaskModel, SharedMAEVisionBackbone


def _resolve_shared_backbone_checkpoint(args) -> str | None:
    candidates = []
    for attr in ("det_backbone_checkpoint", "seg_backbone_checkpoint", "cnt_backbone_checkpoint"):
    ...
    if not candidates:
        return None
    first = candidates[0]
    if any(v != first for v in candidates[1:]):
        print(
            "[multitask/eval][warn] det/seg/cnt backbone checkpoint args are inconsistent; "
            f"using first non-empty value: {first}"
        )
    return first


def _check_full_multitask_load_only(
    checkpoint_path: str,
    *,
    device: str | None,
    model_name: str | None,
    ...
    name = model_name or DEFAULT_MAE_MODEL_NAME
    img_size = int(image_size) if image_size is not None else 448
    dev = torch.device(device) if device else torch.device("cuda" if torch.cuda.is_available() else "cpu")

    MultiTaskModel, SharedMAEVisionBackbone = _import_multitask_models()
...
def _eval_full_model_seg(
    ...
    *,
    ...
    mean, std = get_vit_image_stats(model_name)
...
def _run_eval_full_model(args) -> int:
    ...
    model, meta, _shared_state, _det_state, _seg_state, _cnt_state, dev = _build_full_multitask_model_from_ckpt(
        ...
        backbone_checkpoint=_resolve_shared_backbone_checkpoint(args),
    )
