...
def parse_args():
    ...
    p.add_argument(
        ...
        type=int,
        default=64,
        help="dummy image size used only during SharedMAEVisionBackbone init to infer embed_dim (faster; does not affect eval)",
    )
    p.add_argument("--model-name", type=str, default=DEFAULT_MAE_MODEL_NAME)
...
def _eval_seg(model, *, device: torch.device, stats_dir: Path, args, num_classes: int) -> dict:
    ...
    mean, std = get_vit_image_stats(args.model_name)
