...
def inject_lora_into_mae_mlp(backbone: nn.Module, *, cfg: LoRAConfig) -> int:
    """
    Replace ViT-MAE FFN Linear layers with LoRA-augmented ones.
    - ViTMAEIntermediate / ViTIntermediate: dense
    - ViTMAEOutput / ViTOutput: dense
    Returns the number of Linear layers replaced.
    """
    intermediate_types = {"ViTMAEIntermediate", "ViTIntermediate"}
    output_types = {"ViTMAEOutput", "ViTOutput"}

    replaced = 0
    for module in backbone.modules():
        module_type = module.__class__.__name__
        if module_type in intermediate_types:
            dense = getattr(module, "dense", None)
            if isinstance(dense, nn.Linear) and not isinstance(dense, LoRALinear):
                module.dense = LoRALinear(dense, rank=cfg.rank, alpha=cfg.alpha, dropout=cfg.dropout)
                replaced += 1
        elif module_type in output_types:
            dense = getattr(module, "dense", None)
            if isinstance(dense, nn.Linear) and not isinstance(dense, LoRALinear):
                module.dense = LoRALinear(dense, rank=cfg.rank, alpha=cfg.alpha, dropout=cfg.dropout)
                replaced += 1
    ...
    if replaced == 0:
        raise RuntimeError("Injected 0 LoRA layers; unexpected ViT-MAE FFN structure")
    return int(replaced)


def inject_lora_into_clip_mlp(backbone: nn.Module, *, cfg: LoRAConfig) -> int:
    return inject_lora_into_mae_mlp(backbone, cfg=cfg)
