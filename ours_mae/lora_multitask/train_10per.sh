...
# Optional: override ViT-MAE backbone source or local checkpoint file.
...
SAVE_DIR="${SAVE_DIR:-runs/lora_full}"
...
TEST_STAGE1_CKPT="${TEST_STAGE1_CKPT:-${SAVE_DIR}/best_combo.pt}"
TEST_STAGE2_CKPT="${TEST_STAGE2_CKPT:-${TEST_STAGE1_CKPT}}"
...
run_auto_test_checkpoint() {
  local ckpt_path_raw="$1"
  local ckpt_label="$2"
  local ckpt_path=""
  if ! ckpt_path="$(resolve_ckpt_path "${ckpt_path_raw}")"; then
    echo "[warn] training finished but ${ckpt_label} checkpoint not found: ${ckpt_path_raw}" >&2
    echo "[warn] searched around SAVE_DIR/PROJECT_ROOT/REPO_ROOT and TEST_CKPT_CANDIDATES=${TEST_CKPT_CANDIDATES}" >&2
    exit 3
  fi
  local -a test_cmd=(bash "${SCRIPT_DIR}/test.sh" "${ckpt_path}")
  echo "[test] ${ckpt_label}: ${test_cmd[*]}"
  if [[ "${LOG_TO_FILE}" == "1" ]]; then
...
if [[ "${AUTO_TEST_AFTER_TRAIN}" == "1" ]]; then
  run_auto_test_checkpoint "${TEST_STAGE1_CKPT}" "stage1_ckpt"
  if [[ "${TEST_STAGE2_CKPT}" != "${TEST_STAGE1_CKPT}" ]]; then
    run_auto_test_checkpoint "${TEST_STAGE2_CKPT}" "stage2_ckpt"
  else
    echo "[test] skip duplicate stage2 eval (same as stage1): ${TEST_STAGE2_CKPT}"
  fi
fi
