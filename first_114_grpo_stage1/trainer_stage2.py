...
def run_stage2(
    *,
    ...
    best_epoch = None
    best_loss_weights = (1.0 / 3.0, 1.0 / 3.0, 1.0 / 3.0)
    ...
    def compute_task_loss_and_grads(task_name: str, batch):
        ...
        grads = torch.autograd.grad(loss, theta_params, retain_graph=False, allow_unused=True)
        ...
        with torch.no_grad():
            expert_a, expert_b = state_tracker._expert_grad_matrices(grad_a, grad_b, device=device)
            expert_a = state_tracker._sync_matrix(expert_a, use_ddp, world_size)
            expert_b = state_tracker._sync_matrix(expert_b, use_ddp, world_size)
            task_state = state_tracker._encode_task(expert_a, expert_b)
        return loss, grads, task_state
    ...
    for epoch in range(1, int(args.stage2_epochs) + 1):
        ...
        total_loss = 0.0
        steps = 0
        cnt_epoch_start = [param.detach().float().clone() for param in cnt_head_params]
        ...
        for step, primary_batch in enumerate(train_loaders[primary_task], start=1):
            ...
            final_grads = combine_weighted_grads(task_grads, weights)
            ...
            step_loss = (
                current_weights[0] * step_det_loss
                + current_weights[1] * step_seg_loss
                + current_weights[2] * step_cnt_loss
            )
            ...
            if args.log_interval and step % int(args.log_interval) == 0:
                print(
                    ...
                    f"w [{current_weights[0]:.3f}, {current_weights[1]:.3f}, {current_weights[2]:.3f}]"
                )
            if debug_timing and step % timing_interval == 0:
                print(
        ...
        cnt_param_delta = param_delta_l2_norm(cnt_head_params, cnt_epoch_start)
        print(
            f"[stage2] epoch {epoch}/{int(args.stage2_epochs)} | "
            f"train {total_loss/max(steps,1):.4f} | "
            f"w [{current_weights[0]:.3f}, {current_weights[1]:.3f}, {current_weights[2]:.3f}] | "
            f"cnt_param_delta_l2 {cnt_param_delta:.6e}"
        )
