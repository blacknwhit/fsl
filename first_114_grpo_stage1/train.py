...
def main() -> None:
    ...
    if warmup_load_path is not None and warmup_load_path.is_file():
        if is_main_process:
            load_multitask_checkpoint(str(warmup_load_path), model=model, map_location=device)
    ...
    policy = DirichletWeightGenerator(
        ...
        prior_weights=parse_loss_weights(args.policy_weight_prior),
    ).to(device)
    if use_ddp:
        if warmup_loaded:
            dist.barrier()
    ...
    run_stage2(
        ...
        theta_params=theta_params,
        cnt_head_params=cnt_head_params,
        train_loaders=train_loaders,
        val_loaders=stage2_val_loaders,
        primary_task=primary_task,
        device=device,
