...
CKPT_DEFAULT="/data/xiangyuyue/ULLM-zf/fsl-20260209/runs/113_tadformer_10per/best_combo.pt"
CKPT="${1:-$CKPT_DEFAULT}"
...
if [[ -d "${PROJECT_ROOT}" && -f "${PROJECT_ROOT}/113_tadformer/eval.py" ]]; then
  cd "${PROJECT_ROOT}"
else
...
cmd=(
  "$PYTHON_BIN" -m "113_tadformer.eval"
  --checkpoint "$CKPT"
  --tasks "det,seg,cnt"
  --stats-dir "$STATS_DIR"
  --device "$DEVICE"
