...
if [[ -d "${PROJECT_ROOT}" && -f "${PROJECT_ROOT}/113_tadformer/train.py" ]]; then
  cd "${PROJECT_ROOT}"
else
...
CNT_BACKBONE_GRAD_MULT="${CNT_BACKBONE_GRAD_MULT:-1.0}"

TAD_RANK="${TAD_RANK:-16}"
TAD_SCALE="${TAD_SCALE:-4.0}"
TAD_DROPOUT="${TAD_DROPOUT:-0.05}"
TAD_SPECIALIZE_BLOCKS="${TAD_SPECIALIZE_BLOCKS:-5,11,17,23}"
TAD_FILTER_KERNEL="${TAD_FILTER_KERNEL:-3}"
TAD_QKV_ENABLED="${TAD_QKV_ENABLED:-1}"
TAD_PROJ_ENABLED="${TAD_PROJ_ENABLED:-1}"
TAD_FC1_ENABLED="${TAD_FC1_ENABLED:-1}"
TAD_FC2_ENABLED="${TAD_FC2_ENABLED:-1}"
TAD_PROMPT_ENABLED="${TAD_PROMPT_ENABLED:-1}"
TAD_PROMPT_LEN="${TAD_PROMPT_LEN:-1}"
TAD_TPC_ENABLED="${TAD_TPC_ENABLED:-1}"
TAD_PROMPT_UPDATE_RANK="${TAD_PROMPT_UPDATE_RANK:-64}"
...
SAVE_DIR="${SAVE_DIR:-runs/113_tadformer_10per}"
...
TRAIN_MODULE="${TRAIN_MODULE:-113_tadformer.train}"
...
ARGS=(
  ...
  --cnt-backbone-grad-mult "${CNT_BACKBONE_GRAD_MULT}"
  ...
  --tad-rank "${TAD_RANK}"
  --tad-scale "${TAD_SCALE}"
  --tad-dropout "${TAD_DROPOUT}"
  --tad-specialize-blocks "${TAD_SPECIALIZE_BLOCKS}"
  --tad-filter-kernel "${TAD_FILTER_KERNEL}"
  --tad-prompt-len "${TAD_PROMPT_LEN}"
  --tad-prompt-update-rank "${TAD_PROMPT_UPDATE_RANK}"
)
...
if [[ "${GRAD_CHECKPOINTING}" == "1" ]]; then ARGS+=( --grad-checkpointing ); else ARGS+=( --no-grad-checkpointing ); fi
if [[ "${AMP}" == "1" ]]; then ARGS+=( --amp ); fi
if [[ "${TAD_QKV_ENABLED}" == "1" ]]; then ARGS+=( --tad-qkv-enabled ); else ARGS+=( --tad-qkv-disabled ); fi
if [[ "${TAD_PROJ_ENABLED}" == "1" ]]; then ARGS+=( --tad-proj-enabled ); else ARGS+=( --tad-proj-disabled ); fi
if [[ "${TAD_FC1_ENABLED}" == "1" ]]; then ARGS+=( --tad-fc1-enabled ); else ARGS+=( --tad-fc1-disabled ); fi
if [[ "${TAD_FC2_ENABLED}" == "1" ]]; then ARGS+=( --tad-fc2-enabled ); else ARGS+=( --tad-fc2-disabled ); fi
if [[ "${TAD_PROMPT_ENABLED}" == "1" ]]; then ARGS+=( --tad-prompt-enabled ); else ARGS+=( --tad-prompt-disabled ); fi
if [[ "${TAD_TPC_ENABLED}" == "1" ]]; then ARGS+=( --tad-tpc-enabled ); else ARGS+=( --tad-tpc-disabled ); fi
