...
if __package__:
    ...
    from .utils import choose_primary, infinite_loader, parse_int_list, parse_loss_weights, save_multitask_checkpoint
else:
    package_root = Path(__file__).resolve().parent.parent
    if str(package_root) not in sys.path:
        sys.path.insert(0, str(package_root))
    models_module = importlib.import_module("113_tadformer.models")
    utils_module = importlib.import_module("113_tadformer.utils")
    ...
    parse_int_list = utils_module.parse_int_list
...
def parse_args():
    parser = argparse.ArgumentParser(description="Train the 113_tadformer multitask baseline")
    ...
    parser.add_argument("--save-dir", type=str, default="runs/113_tadformer")
    ...
    parser.add_argument("--tad-rank", type=int, default=16)
    parser.add_argument("--tad-scale", type=float, default=4.0)
    parser.add_argument("--tad-dropout", type=float, default=0.05)
    parser.add_argument("--tad-specialize-blocks", type=str, default="5,11,17,23")
    parser.add_argument("--tad-filter-kernel", type=int, default=3)
    parser.add_argument("--tad-prompt-len", type=int, default=1)
    parser.add_argument("--tad-prompt-update-rank", type=int, default=64)

    tad_qkv = parser.add_mutually_exclusive_group()
    tad_qkv.add_argument("--tad-qkv-enabled", dest="tad_qkv_enabled", action="store_true")
    tad_qkv.add_argument("--tad-qkv-disabled", dest="tad_qkv_enabled", action="store_false")
    parser.set_defaults(tad_qkv_enabled=True)

    tad_proj = parser.add_mutually_exclusive_group()
    tad_proj.add_argument("--tad-proj-enabled", dest="tad_proj_enabled", action="store_true")
    tad_proj.add_argument("--tad-proj-disabled", dest="tad_proj_enabled", action="store_false")
    parser.set_defaults(tad_proj_enabled=True)

    tad_fc1 = parser.add_mutually_exclusive_group()
    tad_fc1.add_argument("--tad-fc1-enabled", dest="tad_fc1_enabled", action="store_true")
    tad_fc1.add_argument("--tad-fc1-disabled", dest="tad_fc1_enabled", action="store_false")
    parser.set_defaults(tad_fc1_enabled=True)

    tad_fc2 = parser.add_mutually_exclusive_group()
    tad_fc2.add_argument("--tad-fc2-enabled", dest="tad_fc2_enabled", action="store_true")
    tad_fc2.add_argument("--tad-fc2-disabled", dest="tad_fc2_enabled", action="store_false")
    parser.set_defaults(tad_fc2_enabled=True)

    tad_prompt = parser.add_mutually_exclusive_group()
    tad_prompt.add_argument("--tad-prompt-enabled", dest="tad_prompt_enabled", action="store_true")
    tad_prompt.add_argument("--tad-prompt-disabled", dest="tad_prompt_enabled", action="store_false")
    parser.set_defaults(tad_prompt_enabled=True)

    tad_tpc = parser.add_mutually_exclusive_group()
    tad_tpc.add_argument("--tad-tpc-enabled", dest="tad_tpc_enabled", action="store_true")
    tad_tpc.add_argument("--tad-tpc-disabled", dest="tad_tpc_enabled", action="store_false")
    parser.set_defaults(tad_tpc_enabled=True)
...
def _eval_cnt_loss(
    ...
    *,
    ...
    for images, density in loader:
        ...
        with torch.amp.autocast(autocast_device, enabled=amp):
            pred_density, pred_counts = model("cnt", images, cnt_backbone_grad_mult=cnt_backbone_grad_mult)
...
def main():
    ...
    shared = SharedDinoV3Backbone(
        ...
        tad_rank=int(args.tad_rank),
        tad_scale=float(args.tad_scale),
        tad_dropout=float(args.tad_dropout),
        tad_specialize_blocks=parse_int_list(args.tad_specialize_blocks),
        tad_filter_kernel=int(args.tad_filter_kernel),
        tad_qkv_enabled=bool(args.tad_qkv_enabled),
        tad_proj_enabled=bool(args.tad_proj_enabled),
        tad_fc1_enabled=bool(args.tad_fc1_enabled),
        tad_fc2_enabled=bool(args.tad_fc2_enabled),
        tad_prompt_enabled=bool(args.tad_prompt_enabled),
        tad_prompt_len=int(args.tad_prompt_len),
        tad_tpc_enabled=bool(args.tad_tpc_enabled),
        tad_prompt_update_rank=int(args.tad_prompt_update_rank),
    ...
    if use_ddp:
        ...
        model = torch.nn.parallel.DistributedDataParallel(
            ...
            # Each task now runs as its own forward/backward pair before the shared
            # optimizer step. Individual task graphs do not touch every parameter,
            # so DDP needs unused-parameter discovery for each pair.
    ...
    if is_main_process:
        tad_params = sum(
            ...
            and (
                "lora_shared_" in name
                or "task_filters" in name
                or "tad_task_prompts" in name
                or "tad_prompt_updaters" in name
                or "tpc_" in name
            )
        )
        head_params = sum(
        ...
        print(f"[train] tad params: {tad_params}")
    ...
    for epoch in range(1, total_epochs + 1):
        ...
        for step, primary_batch in enumerate(train_loaders[primary_task], start=1):
            ...
            with torch.amp.autocast(autocast_device, enabled=bool(args.amp)):
                ...
                det_weighted_loss = float(loss_weights[0]) * det_loss
            scaler.scale(det_weighted_loss).backward()

            with torch.amp.autocast(autocast_device, enabled=bool(args.amp)):
                seg_logits = model("seg", seg_images)
                seg_loss = F.cross_entropy(seg_logits, seg_masks)
                seg_weighted_loss = float(loss_weights[1]) * seg_loss
            scaler.scale(seg_weighted_loss).backward()

            with torch.amp.autocast(autocast_device, enabled=bool(args.amp)):
                pred_density, pred_counts = model(
                    "cnt",
                    cnt_images,
                ...
                cnt_weighted_loss = float(loss_weights[2]) * cnt_loss
            scaler.scale(cnt_weighted_loss).backward()

            total_loss = det_weighted_loss + seg_weighted_loss + cnt_weighted_loss
        ...
        print(f"[train] epoch {epoch}/{total_epochs} | train {total_loss_sum / max(steps, 1):.4f}")
    ...
    if is_main_process:
        ...
        save_multitask_checkpoint(
            ...
            sel_config={},
            train_strategy="joint_tadformer_single_scale",
        )
