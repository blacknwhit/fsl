...
def _infer_lora_config_from_shared_state(shared_state: Dict) -> Dict:
    ...
    _, weight_a = _find_first_tensor(shared_state, ".lora_A")
    if hasattr(weight_a, "shape") and len(weight_a.shape) >= 1:
        config["lora_rank"] = int(weight_a.shape[0])
    return config
...
def parse_args():
    parser = argparse.ArgumentParser(description="Evaluate the 113_pivrg multitask checkpoint")
