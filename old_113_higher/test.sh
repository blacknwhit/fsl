...
CKPT_DEFAULT="/data/xiangyuyue/ULLM-zf/fsl-20260209/runs/test_pertask_realours_10per_2_24_metalearnw_leakrl_nonorm/best_combo.pt"
CKPT="${1:-$CKPT_DEFAULT}"

SCRIPT_DIR="$(cd "$(dirname "${BASH_SOURCE[0]}")" && pwd)"
REPO_ROOT="$(cd "${SCRIPT_DIR}/.." && pwd)"

# Project root that contains the `113_higher/` package.
PROJECT_ROOT_DEFAULT="/data/xiangyuyue/ULLM-zf/fsl-20260209"
PROJECT_ROOT="${PROJECT_ROOT:-${PROJECT_ROOT_DEFAULT}}"

if [[ -d "${PROJECT_ROOT}" && -f "${PROJECT_ROOT}/113_higher/eval.py" ]]; then
  cd "${PROJECT_ROOT}"
else
...
EVAL_SCRIPT_DEFAULT="${PROJECT_ROOT}/113_higher/eval.py"
