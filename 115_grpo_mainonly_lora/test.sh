...
CKPT_DEFAULT="${CKPT_DEFAULT:-"/data/xiangyuyue/ULLM-zf/fsl-20260209/runs/ours_lora_loramoe_100per/best_combo.pt"}"
CKPT="${1:-$CKPT_DEFAULT}"

# Project root that contains the current `115_grpo_mainonly_lora/` package.
PROJECT_ROOT_DEFAULT="/data/xiangyuyue/ULLM-zf/fsl-20260209"
PROJECT_ROOT="${PROJECT_ROOT:-${PROJECT_ROOT_DEFAULT}}"

if [[ -d "${PROJECT_ROOT}" && -f "${PROJECT_ROOT}/115_grpo_mainonly_lora/eval.py" ]]; then
  cd "${PROJECT_ROOT}"
else
...
# while also keeping direct dinov3 import compatibility.
...
EVAL_SCRIPT_DEFAULT="${PROJECT_ROOT}/115_grpo_mainonly_lora/eval.py"
...
if [[ ! -f "$EVAL_SCRIPT" ]]; then
  ...
  echo "Set EVAL_SCRIPT=/path/to/115_grpo_mainonly_lora/eval.py" >&2
  exit 2
fi
...
# i.e. the real model produced by 115_grpo_mainonly_lora/train.py (including LoRA-MoE routing and optional plain LoRA).
