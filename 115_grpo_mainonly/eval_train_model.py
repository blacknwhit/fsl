...
def _import_multitask_models():
    try:
        ...
        models_path = module_root / "models.py"
        module_name = "_grpo_mainonly_local_models"
        spec = importlib.util.spec_from_file_location(module_name, models_path)
        if spec is None or spec.loader is None:
            raise ImportError(f"Cannot load local models.py from {models_path}")
        module = importlib.util.module_from_spec(spec)
        sys.modules[module_name] = module
        spec.loader.exec_module(module)
        try:
            MultiTaskModel = module.MultiTaskModel
            SharedDinoV3Backbone = module.SharedDinoV3Backbone
        except AttributeError as exc:
            raise ImportError("Local models.py does not define MultiTaskModel/SharedDinoV3Backbone") from exc
