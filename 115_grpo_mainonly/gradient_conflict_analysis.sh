...
if [[ -d "${PROJECT_ROOT}" && -f "${PROJECT_ROOT}/115_grpo_mainonly/gradient_conflict_analysis.py" ]]; then
  cd "${PROJECT_ROOT}"
else
...
SCRIPT_PATH="${PROJECT_ROOT}/115_grpo_mainonly/gradient_conflict_analysis.py"

CHECKPOINT_DEFAULT="${PROJECT_ROOT}/runs/115_mainonly_1_10per/best_combo.pt"
CHECKPOINT="${CHECKPOINT:-${CHECKPOINT_DEFAULT}}"
OUTPUT_DIR_DEFAULT="$(dirname "${CHECKPOINT}")/gradient_conflict_vis"
