...
class RoutingCollector:
    def __init__(
        ...
        *,
        ...
        self._original_forwards: list[tuple[object, object]] = []

    def attach(self, model) -> None:
        for ext_block, internal_block in self.block_map.items():
            module = model.shared.lora_moes[int(internal_block)]
            original_forward = module.forward
            module.forward = MethodType(self._build_wrapped_forward(ext_block, original_forward), module)
    ...
    def _build_wrapped_forward(self, ext_block: int, original_forward):
        def _forward(module, *args, **kwargs):
            if args:
                ...
                raise RuntimeError("Cannot find input tensor while collecting routes")
            ...
            if "task_id" in kwargs:
                ...
                raise RuntimeError("Cannot find task_id while collecting routes")
...
def _normalize_rows_to_frequency(matrix: torch.Tensor) -> torch.Tensor:
    values = matrix.to(torch.float32)
    row_sums = values.sum(dim=1, keepdim=True)
    safe_den = torch.where(row_sums > 0, row_sums, torch.ones_like(row_sums))
    normalized = values / safe_den
    return torch.where(row_sums > 0, normalized, torch.zeros_like(normalized))
...
def _render_heatmap(
    *,
    block_external: int,
    block_internal: int,
    shared_freqs: torch.Tensor,
    private_freqs: torch.Tensor,
    ...
    shared = shared_freqs.detach().cpu()
    private = private_freqs.detach().cpu()
    ...
    fig.suptitle(
        f"Block {block_external:02d} routing frequency after row-wise normalization (internal block {block_internal})",
        fontsize=12,
    )
    fig.tight_layout()
...
def main() -> int:
    ...
    for ext_block, internal_block in block_map.items():
        ...
        shared_freqs = _normalize_rows_to_frequency(shared_counts)
        private_freqs = _normalize_rows_to_frequency(private_counts)

        block_tag = f"block_{ext_block:02d}"
        shared_counts_path = output_dir / f"{block_tag}_shared_counts.csv"
        shared_freq_path = output_dir / f"{block_tag}_shared_frequency.csv"
        private_counts_path = output_dir / f"{block_tag}_private_counts.csv"
        private_freq_path = output_dir / f"{block_tag}_private_frequency.csv"
        ...
        _write_matrix_csv(shared_freq_path, row_labels=TASK_NAMES, col_labels=shared_labels, matrix=shared_freqs)
        _write_matrix_csv(private_counts_path, row_labels=TASK_NAMES, col_labels=private_labels, matrix=private_counts)
        _write_matrix_csv(private_freq_path, row_labels=TASK_NAMES, col_labels=private_labels, matrix=private_freqs)
        _render_heatmap(
        ...
        block_outputs[str(ext_block)] = {
            ...
            "shared_frequency_csv": str(shared_freq_path.resolve()),
            "private_counts_csv": str(private_counts_path.resolve()),
            "private_frequency_csv": str(private_freq_path.resolve()),
    ...
    summary = {
        ...
        "normalization": {
            "shared": "row-wise count / sum(counts) over shared experts",
            "private": "row-wise count / sum(counts) over private experts",
        },
