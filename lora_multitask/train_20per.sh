...
SAVE_DIR="${SAVE_DIR:-runs/lora_20per}"
  ...
  for part in "${parts[@]}"; do
    case "$part" in
      ...
      "")
        ;;
      *)
        echo "ERROR: TASKS must be a comma-separated subset of det,seg,cnt; got: ${raw}" >&2
        exit 1
        ;;
    esac
  done
  if [[ "${#items[@]}" -eq 0 ]]; then
    echo "ERROR: TASKS selects no tasks." >&2
    exit 1
  fi
  local joined
...
TRAIN_TASK_FLAGS=()
if [[ ",${TASKS}," != *",det,"* ]]; then TRAIN_TASK_FLAGS+=(--disable-det); fi
if [[ ",${TASKS}," != *",seg,"* ]]; then TRAIN_TASK_FLAGS+=(--disable-seg); fi
if [[ ",${TASKS}," != *",cnt,"* ]]; then TRAIN_TASK_FLAGS+=(--disable-cnt); fi
...
BEST_CKPT="$(cd "${SAVE_DIR}" && pwd)/best_combo.pt"
if [[ "${AUTO_TEST_AFTER_TRAIN}" == "1" ]]; then
  ...
  echo "[test] TEST_TASKS=${TASKS} ${TEST_CMD[*]}"
  if [[ "${LOG_TO_FILE}" == "1" ]]; then
    TEST_TASKS="${TASKS}" "${TEST_CMD[@]}" 2>&1 | tee -a "${LOG_FILE}"
  else
    TEST_TASKS="${TASKS}" "${TEST_CMD[@]}"
  fi
fi
