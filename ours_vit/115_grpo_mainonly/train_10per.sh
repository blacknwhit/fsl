...
# HuggingFace mirror/offline knobs for timm pretrained backbone download.
# Common CN mirror: https://hf-mirror.com
HF_ENDPOINT="${HF_ENDPOINT:-https://hf-mirror.com}"
HF_HUB_CACHE="${HF_HUB_CACHE:-${HOME}/.cache/huggingface/hub}"
TIMM_FORCE_OFFLINE="${TIMM_FORCE_OFFLINE:-0}" # 1=never try online pretrained download
...
MODEL_NAME="${MODEL_NAME:-vit_large_patch16_224.orig_in21k}"
...
TEST_BEST_CKPT="${TEST_BEST_CKPT:-${SAVE_DIR}/best_combo.pt}"
TEST_CKPT_CANDIDATES="${TEST_CKPT_CANDIDATES:-stage1_best.pt,best_combo.pt,stage2_best.pt,final.pt,last.pt}"
...
# Configure HuggingFace endpoint/cache for timm model downloads.
...
if [[ "${TIMM_FORCE_OFFLINE}" == "1" ]]; then
  export HF_HUB_OFFLINE=1
fi
echo "[hf] HF_ENDPOINT=${HF_ENDPOINT:-<default>}"
echo "[hf] HF_HUB_CACHE=${HF_HUB_CACHE:-<default>}"
echo "[hf] TIMM_FORCE_OFFLINE=${TIMM_FORCE_OFFLINE}"
...
resolve_ckpt_path() {
  ...
  if [[ "${raw}" != /* ]]; then
    ...
    candidates+=("${SAVE_DIR}/${raw}")
  fi

  base="$(basename "${raw}")"
  candidates+=("${SAVE_DIR}/${base}")

  IFS=',' read -r -a extra <<< "${TEST_CKPT_CANDIDATES:-}"
  for c in "${extra[@]}"; do
    c="${c// /}"
    [[ -z "${c}" ]] && continue
    candidates+=("${SAVE_DIR}/${c}")
    candidates+=("${PROJECT_ROOT}/${c}")
    candidates+=("${REPO_ROOT}/${c}")
  done
...
run_post_train_eval_sequence() {
  local stage1_raw="${TEST_STAGE1_CKPT}"
  local best_raw="${TEST_BEST_CKPT}"
  local stage1_ckpt=""
  local best_ckpt=""

  if ! stage1_ckpt="$(resolve_ckpt_path "${stage1_raw}")"; then
    echo "[post-train] cannot resolve stage1 checkpoint path: ${stage1_raw}" >&2
    return 2
  fi
  if ! best_ckpt="$(resolve_ckpt_path "${best_raw}")"; then
    echo "[post-train] cannot resolve best checkpoint path: ${best_raw}" >&2
    return 2
  fi

  echo "[post-train] eval #1 stage1 checkpoint: ${stage1_ckpt}"
  run_post_train_eval "${stage1_ckpt}"
  echo "[post-train] eval #2 final best checkpoint: ${best_ckpt}"
  run_post_train_eval "${best_ckpt}"
}
