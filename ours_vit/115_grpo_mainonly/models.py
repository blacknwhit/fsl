...
try:
    from timm_vit_utils import (
        DEFAULT_TIMM_MODEL_NAME,
        build_timm_vit_backbone,
        extract_patch_tokens,
        finalize_timm_vit_tokens,
        forward_timm_vit_prelude,
        get_patch_size,
        get_num_prefix_tokens,
    )
except ImportError:
    from ours_vit.timm_vit_utils import (
        DEFAULT_TIMM_MODEL_NAME,
        build_timm_vit_backbone,
        extract_patch_tokens,
        finalize_timm_vit_tokens,
...
class SharedDinoV3Backbone(nn.Module):
    ...
    def _forward_features_with_lora_moe(self, x: torch.Tensor, task_id: int) -> dict:
        ...
        x = forward_timm_vit_prelude(self.backbone, x)
        for wrapped_block in self.wrapped_blocks:
            x = wrapped_block(x, task_id=task_id)
        x = finalize_timm_vit_tokens(self.backbone, x)
        return {"x_norm_patchtokens": x[:, get_num_prefix_tokens(self.backbone) :]}
