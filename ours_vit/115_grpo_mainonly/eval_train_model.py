...
def parse_args():
    ...
    p.add_argument(
        ...
        type=int,
        default=64,
        help="deprecated for timm backbone; ignored and kept only for CLI compatibility",
    )
    p.add_argument("--model-name", type=str, default="vit_large_patch16_224.orig_in21k")
...
def _build_and_load_model(args) -> tuple[object, Dict[str, object], LoadSummary]:
    ...
    model_name = str(args.model_name or ckpt_config.get("model_name") or "vit_large_patch16_224.orig_in21k")
    backbone_image_size = int(args.image_size if args.image_size is not None else ckpt_config.get("image_size", 448))
