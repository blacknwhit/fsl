...
try:
    from timm_vit_utils import DEFAULT_TIMM_MODEL_NAME, build_timm_vit_backbone, extract_patch_tokens, get_patch_size
except ImportError:
    from ours_vit.timm_vit_utils import DEFAULT_TIMM_MODEL_NAME, build_timm_vit_backbone, extract_patch_tokens, get_patch_size


class SharedDinoV3Backbone(nn.Module):
    def __init__(
        self,
        model_name: str = DEFAULT_TIMM_MODEL_NAME,
        image_size: int = 448,
        ...
        *,
        ...
        super().__init__()
        self.backbone = build_timm_vit_backbone(
            model_name=str(model_name or DEFAULT_TIMM_MODEL_NAME),
        ...
        self.embed_dim = int(getattr(self.backbone, "num_features", getattr(self.backbone, "embed_dim")))
        self.patch_size = get_patch_size(self.backbone)
        ...
        if self.use_lora:
            ...
            expected = int(len(getattr(self.backbone, "blocks", []))) * 2
            if expected > 0 and replaced != expected:
                raise RuntimeError(f"Expected {expected} timm ViT FFN Linear layers to be replaced, got {replaced}")
    ...
    def forward_features(
        ...
        *,
        ...
        return {"x_norm_patchtokens": extract_patch_tokens(self.backbone, tokens)}
