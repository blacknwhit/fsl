...
# Optional: local backbone checkpoint override. Leave empty to use timm pretrained weights.
...
TEST_CKPT="${TEST_CKPT:-}"
TEST_CKPT_CANDIDATES="${TEST_CKPT_CANDIDATES:-best_combo.pt,final.pt,last.pt,stage2_best.pt,stage1_best.pt}"
...
if [[ "${AUTO_TEST_AFTER_TRAIN}" == "1" ]]; then
  TEST_TARGET_CKPT="${TEST_CKPT:-${BEST_CKPT}}"
  RESOLVED_TEST_CKPT=""
  if ! RESOLVED_TEST_CKPT="$(resolve_ckpt_path "${TEST_TARGET_CKPT}")"; then
    echo "[warn] training finished but checkpoint not found: ${TEST_TARGET_CKPT}" >&2
    echo "[warn] searched around SAVE_DIR/PROJECT_ROOT/REPO_ROOT and TEST_CKPT_CANDIDATES=${TEST_CKPT_CANDIDATES}" >&2
    exit 3
  fi
  TEST_CMD=(bash "${SCRIPT_DIR}/test.sh" "${RESOLVED_TEST_CKPT}")
  echo "[test] ${TEST_CMD[*]}"
  if [[ "${LOG_TO_FILE}" == "1" ]]; then
