...
_TASK_NAME_ORDER = ("det", "seg", "cnt")
_SELECTION_LABEL_ORDER = ("all", "det", "seg", "cnt", "none")
_UPDATE_MODE_ORDER = ("all", "shared", "none")


@dataclass(frozen=True)
class MatrixwiseSharedGradStats:
    pair_scores: Dict[str, float]
    positive_pair_counts: Dict[str, int]
    task_update_counts: Dict[str, int]
    update_mode_counts: Dict[str, int]
    selection_counts: Dict[str, int]
    selected_labels: Dict[int, str]
    block_selection_counts: Dict[int, Dict[str, int]]
...
def _empty_task_counts() -> Dict[str, int]:
    return {task_name: 0 for task_name in _TASK_NAME_ORDER}


def _empty_selection_counts() -> Dict[str, int]:
    return {label: 0 for label in _SELECTION_LABEL_ORDER}


def _empty_update_mode_counts() -> Dict[str, int]:
    return {mode_name: 0 for mode_name in _UPDATE_MODE_ORDER}


def _weights_from_task_set(
    updated_tasks: tuple[str, ...],
    base_normalized: tuple[float, float, float],
) -> tuple[float, float, float]:
    task_set = set(updated_tasks)
    return (
        base_normalized[0] if "det" in task_set else 0.0,
        base_normalized[1] if "seg" in task_set else 0.0,
        base_normalized[2] if "cnt" in task_set else 0.0,
    )


def _weights_from_pair_softmax_scores(
    local_pair_scores: Dict[str, float],
    base_weights: tuple[float, float, float],
) -> tuple[float, float, float]:
    # A=det, B=seg, C=cnt:
    # x=cos(A,B), y=cos(B,C), z=cos(A,C)
    # softmax([x,y,z]) -> [x1,y1,z1]
    # raw task scores -> [x1+z1, x1+y1, y1+z1]
    # softmax(raw task scores) * prior weights(15,8,1 by default)
    pair_logits = torch.tensor(
        [
            float(local_pair_scores["det_seg"]),
            float(local_pair_scores["seg_cnt"]),
            float(local_pair_scores["det_cnt"]),
        ],
        dtype=torch.float32,
    )
    ...
    task_logits = torch.stack(
        (
            pair_probs[0] + pair_probs[2],
            pair_probs[0] + pair_probs[1],
            pair_probs[1] + pair_probs[2],
        ),
        dim=0,
    )
    task_probs = torch.softmax(task_logits, dim=0)
    prior = torch.tensor(base_weights, dtype=torch.float32)
    final_weights = task_probs * prior
    return float(final_weights[0]), float(final_weights[1]), float(final_weights[2])


def _resolve_matrix_unit_update_rule(local_pair_scores: Dict[str, float]) -> tuple[tuple[str, ...], str, str]:
    positive_pairs = [
        pair_name
        for pair_name in _PAIR_NAME_ORDER
        if float(local_pair_scores[pair_name]) > 0.0
    ]
    if len(positive_pairs) == 3:
        return _TASK_NAME_ORDER, "all", "all"
    if len(positive_pairs) == 2:
        shared_tasks = set(_PAIR_NAME_TO_TASKS[positive_pairs[0]]).intersection(_PAIR_NAME_TO_TASKS[positive_pairs[1]])
        if len(shared_tasks) == 1:
            shared_task = next(iter(shared_tasks))
            return (shared_task,), shared_task, "shared"
    return tuple(), "none", "none"


def _select_dominant_label_for_block(selection_counts: Dict[str, int]) -> str:
    best_label = _SELECTION_LABEL_ORDER[0]
    best_key = int(selection_counts[best_label])
    for label in _SELECTION_LABEL_ORDER[1:]:
        cur_key = int(selection_counts[label])
        if cur_key > best_key:
            best_key = cur_key
            best_label = label
    return best_label
...
def combine_blockwise_top_pair_grads(
    ...
    for block_id, param_indices in shared_block_index.block_param_indices.items():
        ...
        best_pair = _PAIR_ORDER[0]
        best_score = float("-inf")
        for pair in _PAIR_ORDER:
            score = _safe_cosine_similarity(task_vectors[pair[0]], task_vectors[pair[1]])
            if score > best_score:
                best_score = score
                best_pair = pair
        selected_pairs[block_id] = f"{best_pair[0]}_{best_pair[1]}"

        pair_weights = (
            base_normalized[0] if "det" in best_pair else 0.0,
            base_normalized[1] if "seg" in best_pair else 0.0,
            base_normalized[2] if "cnt" in best_pair else 0.0,
        )
...
def combine_matrixwise_top_pair_grads(
...
) -> tuple[tuple[torch.Tensor | None, ...], MatrixwiseSharedGradStats]:
    base_normalized = _normalize_task_weights(base_weights)
    num_params = len(task_grads["det"])
    if len(task_grads["seg"]) != num_params or len(task_grads["cnt"]) != num_params:
        raise ValueError("Task gradient tuples must have identical lengths")
    ...
    for idx in shared_matrix_index.shared_param_indices:
        template_grad = next(
            (
                task_grads[task_name][idx]
                for task_name in _TASK_NAME_ORDER
                if task_grads[task_name][idx] is not None
            ),
            None,
        )
        out[idx] = None if template_grad is None else torch.zeros_like(template_grad.detach())

    pair_scores = _empty_pair_scores()
    positive_pair_counts = _empty_pair_counts()
    task_update_counts = _empty_task_counts()
    update_mode_counts = _empty_update_mode_counts()
    selection_counts = _empty_selection_counts()
    block_selection_counts = {
        block_id: _empty_selection_counts()
    ...
    for matrix_unit in shared_matrix_index.matrix_units:
        ...
        for pair_name, score in local_pair_scores.items():
            ...
            if float(score) > 0.0:
                positive_pair_counts[pair_name] += 1

        updated_tasks = _TASK_NAME_ORDER
        selection_label = "all"
        update_mode = "all"
        selection_counts[selection_label] += 1
        update_mode_counts[update_mode] += 1
        block_selection_counts[matrix_unit.block_id][selection_label] += 1
        for task_name in updated_tasks:
            task_update_counts[task_name] += 1

        local_pair_weights = _weights_from_pair_softmax_scores(local_pair_scores, base_normalized)
    ...
    selected_labels = {
        block_id: _select_dominant_label_for_block(block_selection_counts[block_id])
        for block_id in shared_matrix_index.block_ids
    }
    stats = MatrixwiseSharedGradStats(
        pair_scores=pair_scores,
        positive_pair_counts=positive_pair_counts,
        task_update_counts=task_update_counts,
        update_mode_counts=update_mode_counts,
        selection_counts=selection_counts,
        selected_labels=selected_labels,
        block_selection_counts=block_selection_counts,
    )
