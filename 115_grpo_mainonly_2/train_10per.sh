...
if [[ -d "${PROJECT_ROOT}" && -f "${PROJECT_ROOT}/115_grpo_mainonly_1/train.py" ]]; then
  cd "${PROJECT_ROOT}"
else
...
STAGE2_INIT_CHECKPOINT="${STAGE2_INIT_CHECKPOINT:-"/data/xiangyuyue/ULLM-zf/fsl-20260209/best_combo.pt"}"
...
SAVE_DIR="${SAVE_DIR:-runs/414_jiaquan_10per}"
...
SWANLAB_PROJECT="${SWANLAB_PROJECT:-115_grpo_mainonly_1}"
...
TRAIN_MODULE="${TRAIN_MODULE:-115_grpo_mainonly_1.train}"
...
resolve_post_train_ckpt() {
  if [[ -n "${TEST_CKPT}" ]]; then
    if [[ -f "${TEST_CKPT}" ]]; then
      echo "${TEST_CKPT}"
      return 0
    fi
    echo "[post-train] TEST_CKPT not found: ${TEST_CKPT}" >&2
    return 2
  fi

  local candidate
  local -a candidates
  IFS=',' read -r -a candidates <<< "${TEST_CKPT_CANDIDATES}"
  for candidate in "${candidates[@]}"; do
    candidate="${candidate// /}"
    [[ -z "${candidate}" ]] && continue
    if [[ -f "${SAVE_DIR}/${candidate}" ]]; then
      echo "${SAVE_DIR}/${candidate}"
      return 0
    fi
  done

  echo "[post-train] no checkpoint found for auto test under ${SAVE_DIR}" >&2
  echo "[post-train] checked candidates: ${TEST_CKPT_CANDIDATES}" >&2
  return 2
}
...
if run_training; then
  if [[ "${RUN_TEST_AFTER_TRAIN}" != "1" ]]; then
    ...
    if resolved_test_ckpt="$(resolve_post_train_ckpt)"; then
      echo "[post-train] resolved test checkpoint: ${resolved_test_ckpt}"
      run_post_train_eval "${resolved_test_ckpt}"
    else
      exit $?
    fi
  fi
