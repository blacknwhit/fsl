...
try:
    from .adapters import collect_balance_loss, compute_spectral_regularization
...
def parse_args():
    parser = argparse.ArgumentParser(description="Train the 113_mtlora_vision multitask model")
    ...
    parser.add_argument("--save-dir", type=str, default="runs/113_mtlora_vision")
    ...
    parser.add_argument("--num-shared-experts", type=int, default=13)
    parser.add_argument("--lora-alpha", type=float, default=32.0)
    parser.add_argument("--adapter-dropout", type=float, default=0.05)
    parser.add_argument("--routing-group-size", type=int, default=256)
    parser.add_argument("--blc-alpha", type=float, default=0.01)
    parser.add_argument("--spectral-reg-lambda", type=float, default=0.5)
    parser.add_argument("--spectral-reg-frequency", type=int, default=1)
...
def _count_diag_stats(pred_density: torch.Tensor, gt_density: torch.Tensor, pred_counts: torch.Tensor, gt_counts: torch.Tensor) -> Dict[str, float]:
...
def _state_dict_cpu_clone(state: Dict[str, torch.Tensor]) -> Dict[str, torch.Tensor]:
    ...
    for key, value in state.items():
        out[key] = value.detach().cpu().clone() if torch.is_tensor(value) else value
    return out
...
def main():
    ...
    shared = SharedDinoV3Backbone(
        ...
        num_shared_experts=int(args.num_shared_experts),
        lora_alpha=float(args.lora_alpha),
        adapter_dropout=float(args.adapter_dropout),
        routing_group_size=int(args.routing_group_size),
    ...
    if is_main_process:
        adapter_params = sum(parameter.numel() for name, parameter in model_for_state.shared.named_parameters() if parameter.requires_grad)
        head_params = sum(
            parameter.numel()
        ...
        print(f"[train] adapter params: {adapter_params}")
    ...
    for epoch in range(1, total_epochs + 1):
        ...
        autocast_device = device.type if device.type in {"cuda", "cpu"} else "cuda"
        spectral_due = (
            float(args.spectral_reg_lambda) > 0
            and int(args.spectral_reg_frequency) > 0
            and (epoch - last_spectral_epoch) >= int(args.spectral_reg_frequency)
        )
        ...
        for step, primary_batch in enumerate(train_loaders[primary_task], start=1):
            ...
            with torch.amp.autocast(autocast_device, enabled=bool(args.amp)):
                joint_outputs = model(
                    ...
                    cnt_density=cnt_density,
                    cnt_count_loss_weight=float(args.cnt_count_loss_weight),
                )
                det_loss = joint_outputs["det_loss"]
                seg_loss = joint_outputs["seg_loss"]
                cnt_loss = joint_outputs["cnt_loss"]
                pred_density = joint_outputs["pred_density"]
                pred_counts = joint_outputs["pred_counts"]
                cnt_gt_counts = joint_outputs["gt_counts"]

                balance_loss = collect_balance_loss(model_for_state)
                balance_loss = det_loss.new_zeros(()) if balance_loss is None else balance_loss.to(det_loss.device, dtype=det_loss.dtype)
                reg_loss = float(args.blc_alpha) * balance_loss
                if spectral_due and step == 1:
                    spectral_loss = compute_spectral_regularization(model_for_state)
                    if spectral_loss is not None:
                        reg_loss = reg_loss + float(args.spectral_reg_lambda) * spectral_loss.to(cnt_loss.device, dtype=cnt_loss.dtype)
                        last_spectral_epoch = epoch
                total = (
            ...
            reg_loss_sum += float(reg_loss.detach().item())
            steps += 1
            if args.log_interval and step % int(args.log_interval) == 0:
                ...
                print(
                    ...
                    f"cnt {cnt_loss_sum / max(steps, 1):.4f} reg {reg_loss_sum / max(steps, 1):.4f} | "
                    f"{_format_count_diag(cnt_stats)}"
                )

        print(f"[train] epoch {epoch}/{total_epochs} | train {total_loss / max(steps, 1):.4f}")
        should_validate = int(args.val_every) > 0 and epoch >= validate_start_epoch and epoch % int(args.val_every) == 0
        if not should_validate:
            continue

        val_det = _eval_det_loss(model, val_loaders["det"], device, amp=bool(args.amp))
        val_seg, val_seg_miou = _eval_seg_loss(model, val_loaders["seg"], device, amp=bool(args.amp), num_classes=int(args.seg_num_classes))
        ...
        val_ap50 = _eval_det_ap50_fast(model, val_loaders["det"], device, num_classes=det_num_classes, score_thresh=float(args.det_ap_score_thr))
    ...
    if is_main_process:
        if best_state is not None:
            ...
            metrics = {"best_metric": float('nan'), "best_epoch": int(save_epoch)}
        ...
        model_config.update(
            {
                ...
                "blc_alpha": float(args.blc_alpha),
                "spectral_reg_lambda": float(args.spectral_reg_lambda),
                "spectral_reg_frequency": int(args.spectral_reg_frequency),
                "epochs": int(total_epochs),
                "validation_start_epoch": int(validate_start_epoch),
            }
        )
