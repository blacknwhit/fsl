...
if [[ -d "${PROJECT_ROOT}" && -f "${PROJECT_ROOT}/113_mtlora_vision/train.py" ]]; then
  cd "${PROJECT_ROOT}"
else
...
SAVE_DIR="${SAVE_DIR:-runs/113_mtlora_vision_10per}"
...
NUM_SHARED_EXPERTS="${NUM_SHARED_EXPERTS:-13}"
LORA_ALPHA="${LORA_ALPHA:-32}"
ADAPTER_DROPOUT="${ADAPTER_DROPOUT:-0.05}"
ROUTING_GROUP_SIZE="${ROUTING_GROUP_SIZE:-256}"
BLC_ALPHA="${BLC_ALPHA:-0.01}"
SPECTRAL_REG_LAMBDA="${SPECTRAL_REG_LAMBDA:-0.5}"
SPECTRAL_REG_FREQUENCY="${SPECTRAL_REG_FREQUENCY:-1}"
...
ARGS=(
  ...
  --routing-group-size "${ROUTING_GROUP_SIZE}"
  --blc-alpha "${BLC_ALPHA}"
  --spectral-reg-lambda "${SPECTRAL_REG_LAMBDA}"
  --spectral-reg-frequency "${SPECTRAL_REG_FREQUENCY}"
)
...
if [[ "${NPROC_PER_NODE}" -gt 1 ]]; then
  RUN_CMD=(
    ...
    --module "113_mtlora_vision.train"
    "${ARGS[@]}"
  )
else
  RUN_CMD=("${PYTHON_BIN}" -m "113_mtlora_vision.train" "${ARGS[@]}")
fi
...
if [[ "${AUTO_TEST_AFTER_TRAIN}" == "1" ]]; then
  ...
  bash "${SCRIPT_DIR}/test.sh" "${BEST_CKPT}"
fi
