...
VALIDATE_LAST_EPOCHS="${VALIDATE_LAST_EPOCHS:-0}" # 0=validate every Stage2 epoch
PHI_CKPT="${PHI_CKPT:-/data/xiangyuyue/ULLM-zf/fsl-20260209/runs/32_64/stage1_phi_last.pt}"
PHI_WARMUP_EPOCHS="${PHI_WARMUP_EPOCHS:-5}"
...
TRAIN_MODULE="${TRAIN_MODULE:-113_test_stage2.train}"
...
ARGS=(
  ...
  --validate-last-epochs "${VALIDATE_LAST_EPOCHS}"
  --phi-checkpoint "${PHI_CKPT}"
  --phi-warmup-epochs "${PHI_WARMUP_EPOCHS}"
