...
def parse_args():
    ...
    p.add_argument(
        "--validate-last-epochs",
        type=int,
        default=0,
        help=(
            "Validate only during the last N Stage2 epochs. "
            "0 disables the restriction and validates every Stage2 epoch."
        ),
    )
    p.add_argument(
        "--phi-checkpoint",
        type=str,
        default="/data/xiangyuyue/ULLM-zf/fsl-20260209/runs/32_64/stage1_phi_last.pt",
        help=(
            "Path to pretrained loss-weight generator (phi) checkpoint. "
            "Supports either a full dict containing 'phi_state' or a raw phi state_dict."
        ),
    )
    p.add_argument(
        "--phi-warmup-epochs",
        type=int,
        default=5,
        help="Stage2 warmup epochs using only --loss-weight-bias before enabling phi-generated weights.",
    )
...
def main():
    ...
    if int(args.phi_warmup_epochs) < 0:
        raise ValueError(f"--phi-warmup-epochs must be >= 0, got {args.phi_warmup_epochs}")
    if int(args.validate_last_epochs) < 0:
        raise ValueError(f"--validate-last-epochs must be >= 0, got {args.validate_last_epochs}")
    ...
    phi_ckpt_path = str(getattr(args, "phi_checkpoint", "") or "").strip()
    if phi_ckpt_path:
        if not os.path.isfile(phi_ckpt_path):
            raise FileNotFoundError(f"--phi-checkpoint not found: {phi_ckpt_path}")
        if (not use_ddp) or is_main_process:
            phi_ckpt = torch.load(phi_ckpt_path, map_location="cpu")
            if isinstance(phi_ckpt, dict) and "phi_state" in phi_ckpt:
                phi_state = phi_ckpt["phi_state"]
            else:
                phi_state = phi_ckpt
            if not isinstance(phi_state, dict):
                raise TypeError(
                    f"Invalid phi checkpoint format at {phi_ckpt_path}: expected dict/state_dict, got {type(phi_state)}"
                )
            load_msg = phi_modules.load_state_dict(phi_state, strict=False)
            print(
                "[train] loaded phi checkpoint: "
                f"{phi_ckpt_path} | missing={len(load_msg.missing_keys)} unexpected={len(load_msg.unexpected_keys)}"
            )
            if load_msg.missing_keys:
                print(f"[train][phi] missing keys: {sorted(load_msg.missing_keys)}")
            if load_msg.unexpected_keys:
                print(f"[train][phi] unexpected keys: {sorted(load_msg.unexpected_keys)}")
        if use_ddp:
            dist.barrier()
    ...
    stage1_saved_loss_weights = (
        _current_loss_weights(g_t)
        if stage1_epochs > 0
        else tuple(float(x) for x in base_loss_weights.detach().cpu().tolist())
    )
    if is_main_process:
        torch.save(
            {
                ...
                "loss_weights": stage1_saved_loss_weights,
            },
            stage1_phi_path,
        )
    ...
    stage2_warmup_epochs = int(args.phi_warmup_epochs)
    stage2_validate_last_epochs = int(args.validate_last_epochs)
    stage2_val_start_epoch = 1
    if stage2_validate_last_epochs > 0:
        stage2_val_start_epoch = max(1, stage2_epochs - stage2_validate_last_epochs + 1)
    if is_main_process and (not bool(args.skip_validation)):
        if stage2_validate_last_epochs > 0:
            print(
                f"[stage2] validation schedule: only validate from epoch "
                f"{stage2_val_start_epoch} to {stage2_epochs}"
            )
        else:
            print(f"[stage2] validation schedule: validate every epoch (1..{stage2_epochs})")
    if stage2_warmup_epochs >= stage2_epochs:
        print(
            "[warn] --phi-warmup-epochs >= --stage2-epochs; "
            "stage2 will keep using --loss-weight-bias only."
        )
    for epoch in range(1, stage2_epochs + 1):
        ...
        stage2_use_dynamic_weights = epoch > stage2_warmup_epochs
        if is_main_process:
            mode = "dynamic(phi)" if stage2_use_dynamic_weights else "warmup(loss-weight-bias only)"
            print(
                f"[stage2] epoch {epoch}/{stage2_epochs} | "
                f"weight_mode={mode} | phi_warmup_epochs={stage2_warmup_epochs}"
            )
        model.train()
        total_loss = 0.0
        det_loss_sum = 0.0
        ...
        for step, primary_batch in enumerate(train_full_loaders[primary_full], start=1):
            ...
            if stage2_use_dynamic_weights:
                w_t = _compose_task_weights(_mlp_weights_from_g(*g_t))
            else:
                w_t = base_loss_weights.detach()
            det_loss, seg_loss, cnt_loss, cnt_stats = _compute_losses(
                batches["det"],
                batches["seg"],
                batches["cnt"],
        ...
        if stage2_use_dynamic_weights:
            ...
            w0, w1, w2 = [float(x) for x in base_loss_weights.detach().cpu().tolist()]
        ...
        should_validate_stage2 = (not bool(args.skip_validation)) and (epoch >= stage2_val_start_epoch)
        if should_validate_stage2:
            ...
            if combo_metric > best_metric:
                ...
                if stage2_use_dynamic_weights:
                    ...
                    best_loss_weights = tuple(float(x) for x in base_loss_weights.detach().cpu().tolist())
                if is_main_process:
                    best_state = _state_dict_cpu_clone(model_for_state.state_dict())
                    best_phi_state = _state_dict_cpu_clone(phi_modules.state_dict())
                    print(f"[ckpt] new best cached (stage2 epoch {epoch}, combo {best_metric:.6f})")
        elif is_main_process and (not bool(args.skip_validation)):
            print(
                f"[stage2] epoch {epoch}/{stage2_epochs} | "
                f"skip validation until epoch {stage2_val_start_epoch}"
            )
