...
def parse_args():
    ...
    # Aux loss weights (shared MI only)
    p.add_argument("--moe-mi-loss-shared", type=float, default=0.0)
    mi = p.add_mutually_exclusive_group()
    mi.add_argument("--use-mi-shared", dest="use_mi_shared", action="store_true")
    mi.add_argument("--no-mi-shared", dest="use_mi_shared", action="store_false")
    p.set_defaults(use_mi_shared=None)
    return p.parse_args()
...
def main() -> int:
    ...
    if args.use_mi_shared is not None:
    ...
    shared = SharedDinoV3Backbone(
        ...
        use_mi_shared=bool(use_mi_shared),
