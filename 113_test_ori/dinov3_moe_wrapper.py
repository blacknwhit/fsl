...
class LoRAMoEBlockWrapper(nn.Module):
    ...
    def forward(self, x: Tensor, task_id: int, rope: Optional[Tuple[Tensor, Tensor]] = None) -> Tensor:
        ...
        """
        # We must NOT checkpoint the LoRA-MoE branch itself because it updates internal
        # aux-loss statistics (stateful side effects). Instead, checkpoint only the
        # original block's attention and MLP computations.
...
def wrap_dinov3_blocks_with_lora_moe(
    ...
    Args:
        ...
        use_mi_shared: Whether to enable MI loss on shared experts
...
class DINOv3WithLoRAMoE(nn.Module):
    ...
    def get_aux_loss_and_clear(self) -> Tuple[Tensor, Tensor, Tensor, Tensor]:
        """
