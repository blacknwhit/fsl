...
class SharedDinoV3Backbone(nn.Module):
    ...
    When use_lora_moe=True:
    - Original backbone is frozen
    - LoRA-MoE (private + shared experts) is added parallel to each block's FFN
    """
    ...
    def _setup_lora_moe(
        ...
        """Setup LoRA-MoE: freeze backbone and wrap blocks."""
        ...
        for block in self.backbone.blocks:
            ...
            lora_moe = LoRATaskMoE(
                ...
                use_mi_loss_shared=use_mi_shared,
                acc_aux_loss=True,
            )
    ...
    def get_aux_loss_and_clear(self) -> Tuple[torch.Tensor, torch.Tensor, torch.Tensor, torch.Tensor]:
        """
        Collect auxiliary losses from all LoRA-MoE layers and clear statistics.
        
        IMPORTANT: Call this ONCE per step, after all task forwards, before backward.
        
        Returns:
            Tuple of (cvloss, switchloss, zloss, MI_loss) - aggregated across all layers
        """
        if not self.use_lora_moe:
        ...
        for lora_moe in self.lora_moes:
            cv, switch, z, mi = lora_moe.get_aux_loss_and_clear()
            total_cv = total_cv + cv
            total_switch = total_switch + switch
            total_z = total_z + z
            total_mi = total_mi + mi
        
        return total_cv, total_switch, total_z, total_mi
...
class _DetBackboneAdapter(nn.Module):
    ...
    def forward(self, x: torch.Tensor) -> torch.Tensor:
        ...
        # Pass task_id for LoRA-MoE routing
...
class MultiTaskModel(nn.Module):
    ...
    def forward_seg(self, x: torch.Tensor) -> torch.Tensor:
        ...
        # Use task_id for LoRA-MoE routing
        if self.shared.use_lora_moe:
