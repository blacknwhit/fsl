...
## Summary
- Purpose: Dual-pool LoRA-MoE adapters (task-private + shared experts) with optional shared MI loss and optional automatic loss weighting.
- Core change: backbone is frozen when LoRA-MoE is enabled; routing uses per-task routers for private/shared pools and MI loss on shared experts.
...
- `models.py`
  ...
  - Adds `get_aux_loss_and_clear()` to collect shared MI loss.
...
- `train.py`
  ...
  - Adds shared MI loss controls: `--moe-mi-loss-shared`, `--use-mi-shared`.
...
- `train.sh` and `train_10per.sh`
  - Expose MoE flags and auxiliary loss weights via environment variables.
