...
def test_lora_moe():
    ...
    cv, sw, z, mi = lora_moe.get_aux_loss_and_clear()
    print(f"aux losses: cv={cv:.6f}, switch={sw:.6f}, z={z:.6f}, MI={mi:.6f}")
...
def test_integration():
    ...
    cv_total, sw_total, z_total, mi_total = 0.0, 0.0, 0.0, 0.0
    for moe in lora_moes:
        cv, sw, z, mi = moe.get_aux_loss_and_clear()
        cv_total += cv
        sw_total += sw
        z_total += z
        mi_total += mi
