...
def _infer_mtlora_config_from_shared_state(shared_state: Dict) -> Dict:
    config = {
        "model_name": "dinov3_vitl16",
        "image_size": 448,
        "mtlora_shared_rank": 16,
        "mtlora_task_rank": 2,
        "mtlora_shared_scale": 4.0,
        "mtlora_task_scale": 4.0,
        "mtlora_dropout": 0.05,
        "mtlora_bias": "none",
        "mtlora_shared_mode": "matrix",
        "mtlora_specialize_blocks": [5, 11, 17, 23],
        "mtlora_trainable_scale_shared": False,
        "mtlora_trainable_scale_per_task": False,
        "mtlora_intermediate_specialization": False,
        "mtlora_qkv_enabled": False,
        "mtlora_proj_enabled": False,
        "mtlora_fc1_enabled": False,
        "mtlora_fc2_enabled": False,
        "grad_checkpointing": False,
    }

    _, shared_weight = _find_first_tensor(shared_state, ".lora_shared_A")
    if hasattr(shared_weight, "shape") and len(shared_weight.shape) >= 1:
        config["mtlora_shared_rank"] = int(shared_weight.shape[0])

    _, task_weight = _find_first_tensor(shared_state, ".lora_tasks_A.det")
    if hasattr(task_weight, "shape") and len(task_weight.shape) >= 1:
        config["mtlora_task_rank"] = int(task_weight.shape[0])

    specialize_blocks = set()
    for key in shared_state.keys():
        if not isinstance(key, str) or ".lora_tasks_A." not in key:
            continue
        parts = key.split(".")
        if len(parts) >= 4 and parts[0] == "backbone" and parts[1] == "blocks":
            try:
                specialize_blocks.add(int(parts[2]))
            except ValueError:
                continue
        if ".attn.proj." in key:
            config["mtlora_proj_enabled"] = True
        if ".mlp.fc1." in key:
            config["mtlora_fc1_enabled"] = True
        if ".mlp.fc2." in key:
            config["mtlora_fc2_enabled"] = True
    if specialize_blocks:
        config["mtlora_specialize_blocks"] = sorted(specialize_blocks)

    for key in shared_state.keys():
        if not isinstance(key, str):
            continue
        if ".attn.qkv.lora_shared_A" in key:
            config["mtlora_qkv_enabled"] = True
...
def _infer_model_config(ckpt: Dict) -> Dict:
    ...
    config = _infer_mtlora_config_from_shared_state(shared_state)
...
def _build_model_from_checkpoint(
    ...
    *,
    ...
    shared = SharedDinoV3Backbone(
        ...
        mtlora_shared_rank=int(config["mtlora_shared_rank"]),
        mtlora_task_rank=int(config["mtlora_task_rank"]),
        mtlora_shared_scale=float(config["mtlora_shared_scale"]),
        mtlora_task_scale=float(config["mtlora_task_scale"]),
        mtlora_dropout=float(config["mtlora_dropout"]),
        mtlora_bias=str(config.get("mtlora_bias", "none")),
        mtlora_shared_mode=str(config.get("mtlora_shared_mode", "matrix")),
        mtlora_specialize_blocks=config.get("mtlora_specialize_blocks", [5, 11, 17, 23]),
        mtlora_trainable_scale_shared=bool(config.get("mtlora_trainable_scale_shared", False)),
        mtlora_trainable_scale_per_task=bool(config.get("mtlora_trainable_scale_per_task", False)),
        mtlora_intermediate_specialization=bool(config.get("mtlora_intermediate_specialization", False)),
        mtlora_qkv_enabled=bool(config.get("mtlora_qkv_enabled", True)),
        mtlora_proj_enabled=bool(config.get("mtlora_proj_enabled", True)),
        mtlora_fc1_enabled=bool(config.get("mtlora_fc1_enabled", True)),
        mtlora_fc2_enabled=bool(config.get("mtlora_fc2_enabled", True)),
        grad_checkpointing=False,
    )
...
def parse_args():
    parser = argparse.ArgumentParser(description="Evaluate the 113_mtlora multitask checkpoint")
