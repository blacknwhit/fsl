...
def parse_args():
    parser = argparse.ArgumentParser(description="Train the 113_mtlora multitask baseline")
    ...
    parser.add_argument("--save-dir", type=str, default="runs/113_mtlora")
    ...
    parser.add_argument("--mtlora-shared-rank", type=int, default=16)
    parser.add_argument("--mtlora-task-rank", type=int, default=2)
    parser.add_argument("--mtlora-shared-scale", type=float, default=4.0)
    parser.add_argument("--mtlora-task-scale", type=float, default=4.0)
    parser.add_argument("--mtlora-dropout", type=float, default=0.05)
    parser.add_argument("--mtlora-bias", type=str, default="none", choices=["none", "all", "lora_only"])
    parser.add_argument(
        "--mtlora-shared-mode",
        type=str,
        default="matrix",
        choices=["matrix", "matrixv2", "addition", "lora_only"],
    )
    parser.add_argument("--mtlora-specialize-blocks", type=str, default="5,11,17,23")
...
def main():
    ...
    shared = SharedDinoV3Backbone(
        ...
        mtlora_shared_rank=int(args.mtlora_shared_rank),
        mtlora_task_rank=int(args.mtlora_task_rank),
        mtlora_shared_scale=float(args.mtlora_shared_scale),
        mtlora_task_scale=float(args.mtlora_task_scale),
        mtlora_dropout=float(args.mtlora_dropout),
        mtlora_bias=args.mtlora_bias,
        mtlora_shared_mode=args.mtlora_shared_mode,
        mtlora_specialize_blocks=parse_int_list(args.mtlora_specialize_blocks),
    ...
    if is_main_process:
        ...
        print(f"[train] mtlora params: {adapter_params}")
    ...
    for epoch in range(1, total_epochs + 1):
        ...
        for step, primary_batch in enumerate(train_loaders[primary_task], start=1):
            ...
            scaler.scale(det_loss * float(loss_weights[0])).backward()
            ...
            scaler.scale(seg_loss * float(loss_weights[1])).backward()
            ...
            scaler.scale(cnt_loss * float(loss_weights[2])).backward()
            ...
            total = (
                float(loss_weights[0]) * float(det_loss.detach().item())
                + float(loss_weights[1]) * float(seg_loss.detach().item())
                + float(loss_weights[2]) * float(cnt_loss.detach().item())
            )
            total_loss += total
